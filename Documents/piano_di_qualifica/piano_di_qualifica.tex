...
\subsubsection{CMMI}
Questo modello è focalizzato sulla qualità interna e permette ai fornitori di auotvalutarsi.
...
\item Stimare le caretteristiche del software prodotto nello stadio di sviluppo raggiunto in cui si sta effettuando la valutazione.
\end{itemize}
...
\begin{itemize}
\item textbf{Fase di progettazione:} prevedere eventuali problemi nel software in un periodo temporale successivo al suo rilascio ed quanto sarà facile eseguire eventuali interventi di manutenzione, valutandone l'impatto che avranno nel contesto d'utilizzo.

\item textbf{Fase di collaudo:} confrontare che quanto sarà prodotto sia coerente con le specifiche del committente, analizzando eventuali problemi che possono emergere e non sono stati considerati nella fasse di progettazione.

\item textbf{Periodo successivo alla data di rilascio:} misurare l'impatto del software sull'efficienza e l'efficacia del lavoro svolto dall'utente che ne usufruisce, eventualmente confrontando il prodotto con eventuali concorrenti simili e valutandone potenziali aree di miglioramento, pianificando aggiornamenti mirati e creando, eventualmente, una versione migliorata del prodotto stesso.
\end{itemize}
...
\item textbf{Nell'analisi dei requisiti:} utili per stimare il costo del software e le implicazioni sul ciclo di vita del software, in questa fase si può misurare il numero di requisiti e di Use Case evidenziati.

\item textbf{Nella progettazione:} stimano in modo più accurato il costo del software e alcuni aspetti inerenti alla qualità. In questa fase è possibile quantificare il numero di moduli in uno schema di progettazione e il loro livello di coesione ed accoppiamento.

\item textbg{Nel codice:} come per le misure nella fase di progettazione si hanno stime a livello economico e qualitativo, misurando il numero di linee di codice prodotto, la complessità ciclomatica dello stesso, di coesione funzionale ed eventuali mirure object oriented che valutano l'ereditarietà, il polimorfismo, l'incapsulamento e altre caratteristiche correlate alla programmazione orientata agli oggetti.
\end{itemize}

Mentre le misure esterne sono principalmente basate sul modello di qualità di Boehm:
...
% TODO:
% completare questa sezione con l'esito delle attività di verifica per le attività di analisi e di progettazione architetturale

\section{Esito delle revisioni}
% TODO:
% aggiungere dettaglio esito delle revisioni
=======
...
In segito alle osservazioni evidenziate durante la Revisione dei Requisiti, il team si è prodigato ad analizzare le tematiche e gli errori evidenziati dal committente, modificando i documenti relativi e realizzando una versione migliorata degli stessi.
Viene riportato in dettaglio le modifiche effettuate dal gruppo per risolvere le problematiche evidenziate:

\begin{itemize}
\item \textbf{Analisi dei requisiti}: Aggiunta una lista di distrubizione e un sommario al documento. Riviste sez. 3.3 e 3.4 e vincoli generali che non sono stati riportati nel documento ma citati nel capitolato. Rivisitati e corretti i diagrammi dei casi d'uso evidenziati, eliminando gli errori e le imprecisioni riscontrate, completando o integrando la descrizione dove suggerito. Aggiunte didascalie esplicative a figure sprovviste. Inserito il tracciamento fra requisiti e cadi d'uso.
...
In questa sezione verranno descritte in che modo sono state effettuate le attività di verifica e i loro relativi esiti.

\subsubsection{Analisi dei requisiti}
Le verifiche sono state prevalentemente mirate al tracciamento dei requisiti riportati nel documento Analisi dei requisiti.
Da questa verifica si possono trarre le seguenti conclusioni
\begin{itemize}
\item Tutte le funzioni e caratteristiche emerse nel capitolato e nell'incontro con il committente sono coperte dai requisiti e sono stati rilevati complessivamente X: di cui X funzionali obbligatori, X funzionali desiderabili e X funzionali opzionali. Si rimanda alla tabella X nel documento Analisi dei requisiti 2.0 per maggiori dettagli;

\item I requisiti sono tutti coperti dai relativi use case. Si rimanda alla tabella X nel documento Analisi dei requisiti 2.0 per maggiori dettagli;
\end{itemize}

Per quanto riguarda i rimanenti documenti la priorità del team è stata incentrata sulla qualità generale, la pertinenza dei contenuti rispetto allo scopo stesso del documento e la doverosa correttezza sintattica e ortografica.

\subsubsection {Progettazione Architetturale}
Nella fase di progettazione si è posta particolare attenzione al particolare che ad ogni requisto funzionale espresso durante l'attività di analisi dei requisiti corrisponda una componente architetturale, garantendo pertanto il soddisfacimento dei requisiti stessi.
L'esito di tale tracciamento è consutabile nel documento allegato Specifica tecnica 1.0.
