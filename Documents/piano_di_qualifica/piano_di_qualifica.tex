...
\begin{itemize}
\item[-] \inglese{capability}: adeguatezza di un processo ai suoi scopi (efficacia);
\item[-] \inglese{maturity}: livello di governabilità dei processi;
\item[-] \inglese{model}: requisiti di valutazione del miglioramento;
\item[-] \inglese{integration}: livello di integrazione della struttura del fornitore.
\end{itemize}

Le 24 aree possono essere riassunte in quattro gruppi:\\
\begin{tabular}{>{\bfseries\itshape}lp{.7\textwidth}}
...
\item \inglese{Tag}: indicano i termini inerenti all'anomalia per rendere la ricerca e l'identificazione più rapida ed efficace possibile.
\end{itemize}
...
in totale sono stati rilevati 99 requisiti, di cui 20 funzionali obbligatori, 5 funzionali desiderabili e 46 funzionali facoltativi. Si rimanda al documento \textit{analisi\_dei\_requisiti.3.0.pdf} per maggiori dettagli, relativi anche ai requisiti di qualità, dichiarativi e prestazionali;
