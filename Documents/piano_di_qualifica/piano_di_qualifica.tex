...
\item \texttt{testLoadMissingUserName()} verifica il corretto funzionamento della classe nel caso in cui la richiesta al server non contenga tutti i parametri necessari. Il controllo avviene verificando che venga sollevata un'eccezione di tipo \texttt{IOException}, come ci si aspetta nel caso in cui non tutti i parametri necessari siano forniti al metodo.


\item \texttt{testLoadCorrectUsername()}verifica il corretto funzionamento della classe nel caso in cui la richiesta al server contenga tutti i parametri necessari. boh...
\item
\texttt{testGetData()} boh.
...
\item \texttt{testLoadMissingPassword()} verifica il corretto funzionamento della classe nel caso in cui la richiesta al server non contenga tutti i parametri necessari. Il controllo avviene verificando che venga sollevata un'eccezione di tipo \texttt{IOException}, come ci si aspetta nel caso in cui non tutti i parametri necessari siano forniti al metodo.


\item \texttt{testPasswordLoaderCorrectPassword()}verifica il corretto funzionamento della classe nel caso in cui la richiesta al server contenga tutti i parametri necessari. boh...

Viene controllato il corretto funzionamento del recupero della password e che la strategia di codifica di essa sia invocata una sola volta per effettuare la crittografia della password. Viene inoltre verificato che la password corrisponda alla versione crittografata di quella impostata tramite la richiesta.
...
\begin{itemize}
\item \texttt{testDeleteSuccessfully()} 

\item \texttt{testDeletensuccessfully()} 

\item \texttt{testInsertSuccessfully()} 

\item \texttt{}testInsertUnsuccessfully()

\item \texttt{testUpdateSuccessfully()} 

\item \texttt{testUpdateUnsuccessfully()}
...
\item \texttt{testGetUserDataByEmail()}

\item \texttt{testGetUserDataByEmailEmpty()} 

\item \texttt{testGetUserDataById()} 

\item \texttt{testGetUserDataByIdEmpty()}

\item \texttt{testGetUserDataGeneric()}
...
Tale test si compone di due metodi:
\begin{itemize}
\item \texttt{testGetSessionFactory()} 

\item \texttt{testGetInstance()}
...
\item \texttt{testExecuteWithoutTransaction()}
verifica il comportamento della classe nel momento in cui è possibile avviare una transazione per l'interazione con il database.
...
****************
controlla il comportamento del metodo \texttt{execute} nel caso in cui al momento di effettuare la query verificarsi di un evento eccezionale. In particolare, il test assicura che non sia mai invocato il metodo \texttt{commit()} sulla transazione e che sia invece invocato il \texttt{rollback()}.Inoltre p garantito che durante l'esecuzione del metodo dia aperta una sessione di interazione con il database e che quest'ultima sia finalizzata in modo corretto. Infine, si controlla che non siano mai invocati i metodi sulla lista di chiamate che corrisponde alla query di ricerca con cui è invocato il metodo \texttt{execute}, dal momento che in fase di interrogazione del database avviene un errore.
...
\item \texttt{testExecuteUnableToPerformAction()}
...
Tale test si compone di sette metodi:
\begin{itemize}
\item \texttt{testGetDeleteUtil()} 

\item \texttt{testGetInsertUtil()} 

\item \texttt{testGetUpdateUtil()} 

\item \texttt{testGetCallUtil()} 

\item \texttt{testGetGroupUtil()}

\item \texttt{testGetUserDataUtil()}

\item \texttt{testGetGenericUtil()}
