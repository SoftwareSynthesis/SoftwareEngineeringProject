...
L'analisi statica non comporta l'esecuzione del codice in quanto la verifica avviene tramite lettura da parte di un verificatore (tramite \underline{\inglese{inspection}} e \underline{\inglese{walkthrough}}, descritti più dettagliatamente nella sezione 5.1) o mediante opportuni strumenti software di verifica statica. Gli errori riscontrati mediante tali metodologie dal verificatore non vengono corretti dal soggetto stesso, ma vengono riferiti al progettista o al programmatore al quale comunque rimane il compito di individuazione e correzione degli errori individuati durante il suo lavoro sul codice stesso.
...
Questa tipologia di analisi può rilevare anomalie quali scritture successive su una variabile senza letture intermedie. Al fine di evitare complicazioni inutili per questo tipo di analisi si opterà per sconsigliare l'uso di variabili globali in fase di progettazione e codifica.
...
Per effettuare delle misure efficacemente è nesessario definire un'unità di misura, una scala metrica e il metodo per rilevare tali misure.
...
\begin{itemize}
  ...
  \]
  \item complessità ciclomatica (o complessità condizionale), un indice che misura all'interno di un metodo il numero di cammini distinti che il flusso di controllo può intraprendere nel codice sorgente incrementando un indice di 1 unità per ogni istruzione di \inglese{branch} (\verb+if+, \verb+for+, \verb+while+, \verb+do...while+, \verb+case+, \verb+catch+, operatore condizionale ternario, operatori logici cortocircuitati);
...
Mentre le misure esterne sono principalmente basate sul modello di qualità di Boehm, che si sviluppa mediante approccio \inglese{top-down}: a livello gerarchico nelle parti superiori compaiono gli attributi qualitativi dal punto di vista del committente, mentre nei livelli inferiori gli attributi dal punto di vista del fornitore.
...
\begin{itemize}
	\item spronare i membri del team a ricercare gli errori al termine di un attività svolta, cosi da migliorare la ripetizione della medesima attività nelle successive fasi;
	\item spronare i membri del team ad autogiudicarsi sul proprio operato, scontrandosi (per mezzo di critiche costruttive) con il giudizio altrui;
	\item spronare i membri del team a ricercare tecniche d'utilizzo del software sempre più pratiche ed economiche (in termini di tempistiche).
\end{itemize}

L'obbiettivo finale è quello di attuare, sulla base di tali considerazioni, un miglioramento continuo che si ripeterà per ogni fase, aiutando il team a ``crescere'' non solo in termini di professionalità, ma anche ad imparare ad apprezzare gli sforzi altrui riconoscendo i propri errori e migliorando cosi il livello di comunicazione interno. 
...
\begin{itemize}
	\item In merito ai ruoli svolti in questa fase, che errori pensi di aver fatto?
	\item Come pensi si possano evitare gli errori evidenziati al punto precedenti?
	\item In merito agli strumenti utilizzati, credi sia possibile migliorare il loro utilizzo? Se si come?
\end{itemize}

L'amministratore avrà poi il compito di prendere in considerazione tali documenti, leggerli e riportarne il contenuto nelle sezioni successive utilizzando una forma appropriata.
...
\begin{description}
	\item \textbf{Errori riscontrati}:
		\begin{itemize}
			...
			\item difficoltà nel stimare le risorse in maniera corretta (causa l'inesperienza);
			\item difficoltà nel cogliere la situazione generale dello stato dei lavori;
			...
			\item errori nella creazione dei ticket: non sempre i ticket creati erano di buona ``granularità''. È stato osservato che alcuni ticket risultavano essere eccessivamente onerosi per essere assegnati ad un unico membro del team.
		\end{itemize}
	...
	\item \textbf{Suggerimenti per l'automiglioramento}:
		\begin{itemize}
			...
			\item il rispetto delle pratiche e norme inserite nei documenti deve essere rispettata in maniera disciplinata. Il responsabile non dovrà lasciare che le tempistiche lo portino ad abbandonare una buona norma di lavoro;
			...
			\item controllare in maniera rigorosa i commit dei membri del gruppo, rilasciando commenti appropriati e costruttivi.
		\end{itemize}
\end{description}
...
\begin{description}
	...
	\item \textbf{Suggerimenti per l'automiglioramento}:
		\begin{itemize}
			...
			\item migliorare la documentazione sull'utilizzo di determinati strumenti. In merito alla creazione di template o di script di automatizzazione, tali dovranno essere adeguatamente commentati. Tuttavia in caso di dubbi si invita a consultare chi ha svolto l'attività di amministratore in precedenza. Tale pratica (ovviamente costosa) va però svolta con parsimonia e non per banalità. Il giudizio su quando ciò sia o meno necessario è lasciato agli amministratori stessi con la consapevolezza che ciò crescerà con l'esperienza.		
		\end{itemize}
\end{description}
...
\begin{description}
	...
	\item \textbf{Suggerimenti per l'automiglioramento}:
		\begin{itemize}
			...
			\item dare maggior importanza all'attività di verifica, evitando di ``correre'' a causa di tempistiche stringenti.
		\end{itemize}
\end{description}
...
\begin{description}
	...
	\item \textbf{Concept Draw}: per quanto tale software si sia rivelato pratico ed intuitivo, esso si è anche rivelato più pesante della norma (in particolare in fase di esportazione dei diagrammi). Si è osservato che il rallentamento dovuto alle operazioni più onerose ha portato le medesime ad essere considerate ``snervanti'', e hanno comportato un certo malumore nei membri del team.  Si suggerisce quindi (nei prossimi progetti del team) di prendere in considerazione altri software alternativi.
	\item \textbf{LaTeX}: difficoltà nel suo utilizzo sono dovute ad una scarsa documentazione sugli appunti riportati dall'amministratore in merito al suo utilizzo. Si consiglia ai membri del gruppo di prendere visione di tali appunti con maggior serietà. Evitare in assoluto modalità ``copia-incolla'' di parti di documento. Utilizzare norme appropriate  per la stesura dei documenti, evitando le \inglese{worst practices}.
\end{description}

\subsection{Considerazioni sui rapporti interpersonali}

La presente sotto-sezione intende riassumere i punti focali inerenti i rapporti tra i membri del team. Tali sono da intendere come suggerimenti da rispettare per il rispetto comune. Consigliati, al fine di non far insorgere litigi controproducenti allo sforzo comune.
...
\begin{itemize}
	...
	\item \textbf{Mai mettere alla berlina chi sbaglia}: indubbiamente ogni membro del team, ha delle responsabilità che cambiano in virtù del ruolo che ricopre. Va però ricordato che in caso d'errore non si dovrà mai e poi mai giudicare colpevole e conseguentemente gambizzare un membro del team. Ciò potrebbe si portare chi ha sbagliato a prestare una maggiore attenzione al proprio operato, ma il rischio di  portare il suddetto membro del gruppo, a non esporre le proprie idee per paura di essere zittito è grande, e potrebbe nuocere alla qualità del prodotto finale. Sarà quindi esclusiva responsabilità del responsabile, comunicare in privato gli errori ai relativi ``colpevoli''. Cosi facendo chi sbaglia sarà corretto e si eviterà che questi si senta pubblicamente umiliato.
\end{itemize}
