...
\item \texttt{testOnShowRegistrationPanel()} verifica che venga visualizzato il pannello di registrazione quando il \inglese{presenter} reagisce all'evento \verb'showRegistrationPanel', contrllando tramite un \inglese{mock} di \texttt{PresenterMediator} che sia invocato il metodo \verb'getView'.
...
\textbf{Verifica il requisito:} RSDD2.1.1.\\
...
Infine i requisiti desiderabili sono stati coperti per il 66\% degli stessi, non implementando quindi l'interfaccia multi-lingua e la stima della complessità della \textit{password}, l'ultimo requisito non soddisfatto è condizionato dall'impossibilità sopracitata di effettuare chiamate con più di due utenti, pertanto risulta inutile eliminare un membro della chiamata se non chiudendo effettivamente la stessa.
