...
\end{table}
Il valore complessivo che se ne ricava è quindi una complessità ciclomatica media pari a 1,69 con un massimo di 9 in \texttt{RegistergPresenter.js}.
...
\item \textbf{TM3.04: AddInGroupControllerTest}\\
...
\item \textbf{TM3.08: DeleteGroupControllerTest}\\
...
\item \textbf{TM3.09: GetContactsControllerTest}\\
...
\textbf{Descrizione}: verifica la corretta inizializzazione di \textit{AccountSetting}, ovvero che l'albero sia stato costruito correttamente, e che il contenuto dei vari nodi sia stato inserito coerentemente. Verifica inoltre la corretta inizializazione del \inglese{\underline{form}} per la modifica dei dati personali e l'invio dei dati in forma corretta al server.
Tale verifica è composta da tre test:
\begin{itemize}

\item \texttt{testDisplay()} crea l'elemento \textit{AccountSetting}, estrae la lista dei suoi figli, ne controlla il numero e che ogni nodo sia coerente con quanto atteso.
...
\item \texttt{testSendUserData()} Verifica il metodo sendUserData verificando che venga rilevata o non venga rilevata la mancanza di cambiamenti nei dati dell'utente.
...
\item \textbf{TP1.02:AddressBookPresenterTest}\\  %TODO da aggiornare
...
\item \textbf{TP1.04: CommunicationPresenterTest}\\ %TODO da aggiornare
...
\item \texttt{testCreate()} controlla che il numero dei figli del \textit{Communication} siano esattamente due, che entrambi siano \texttt{<div>} e che il valore dell'attributo \texttt{id} del primo figlio sia ``divCall'' mentre del secondo ``divChat''. Verifica i figli dell'elemento per la visualizzazione delle chiamate (due elementi \texttt{<video>} e un ulteriore elemento \texttt{<div>} per la visualizzazione delle statistiche) e i figli dell'elemento per la visualizzazione delle chat.
...
\item \textbf{TP1.05: ContactPresenterTest}\\
\textbf{Verifica l'oggetto:} \texttt{ContactPresenter}.\\
\textbf{Descrizione}: l'obiettivo di questo test è verificare la corretta creazione del sotto-albero che ha radice nell'elemento \textit{Contact}.\\
Tale verifica è composta da sei test:
\begin{itemize}

\item \texttt{testShowUnblockedOnlineContact()} verifica che un contatto online, presente in rubrica e non bloccato sia visualizzato correttamente. Verifica inoltre che l'albero di visualizzazione di un contatto sia correttamente inizializzato.
...
\item \textbf{TP1.06: GroupPresenterTest}\\   %TODO da aggiornare
...
\textbf{Descrizione}: l'obiettivo di questo test è verificare la corretta istanziazione del \textit{Login} popolato con dei dati coerenti rispetto allo scopo della \inglese{form}.
...
\item \texttt{testRetrivePasswordSuccessfully()} verifica il comportamento se si inserisce la risposta corretta nel \inglese{form} per il recupero della password.

\item \texttt{testRetrivePasswordUnsuccessfully()} verifica il comportamento se si inserisce una risposta errata nel \inglese{form} per il recupero della password.

\item \texttt{testOnInputLoginClick} verifica il comportamento al \inglese{click} sul pulsante di login.
...
\item \texttt{testDestroy()} controlla che venga correttamente eliminato il pannello una volta effettuato il \inglese{logout} al sistema.
...
\textbf{Descrizione}:l'obiettivo di questo test è verificare la corretta creazione del sotto-albero che ha radice nell'elemento \textit{Register}, la possibilità di nascondere correttamente l'elemento \textit{Register}, il corretto recupero dei dati inseriti nella \inglese{form} d'iscrizione e infine i test relativi al recupero delle singole informazioni passate in fase di registrazione.\\
Tale verifica è composta da dieci test:
\begin{itemize}


%TODO AGGIORNATO FIN QUI
...
\item \texttt{testGetPicturePath()} crea una \inglese{form} compilata correttamente con l'immagine utente da inserire, successivamente prova a recuperarla mediante 

\texttt{getPicturePath}, verificando se è stata inserita correttamente (o meno).
...
\textbf{Descrizione}: l'obiettivo di questo test è verificare che sia correttamente creato l'elemento grafico \textit{SearchResult} e il comportamento del presenter nel momento in cui tale pannello deve essere aggiornato. Tale verifica è composta da due test:
\begin{itemize}
\item \texttt{testCreate()} verifica che il pannello sia costruito e visualizzato in accordo con le specifiche della progettazione e, in particolare, che sia presente la lista dei contatti che corrispondono ai risultati della ricerca.
...
TP1.09 & MainPresenterTest &Superato\\
