...
La qualità dei processi, come già accennato ad inizio sezione, è una condizione necessaria per arrivare ad un prodotto di qualità. Il team si pone l'obiettivo di garantire e ottimizzare la qualità dei propri processi mediante l'utilizzo di strumenti necessari per garantirla, pertanto ogni processo sarà definito in modo dettagliato e valutato l'output prodotto. Inoltre verranno attuate procedure di misurazione, valutazione ed eventuale modifica sul processo per perseguire il suo miglioramento mediante una costante collaborazione tra il verificatore e lo sviluppatore del processo stesso.
...
\subsection{Tecniche}
Durante lo svolgimento del progetto il team utilizzerà le tecniche di analisi statica e dinamica (descritte nel paragrafo 4.1.3) per testare il software.
L'analisi statica farà largo uso della matrice delle dipendenze che definisce quali informazioni sono necessarie per avviare una particolare attività (tracciamento requisiti-componenti), facilitando la tracciabilità di entrambi.
Durante le prime verifiche verrà adottata la tecnica di \inglese{walkthrough} in quanto non risultano ancora diponibili dati o statistiche sugli errori più comuni e frequenti. Man mano che il team analizzerà gli errori emersi verrà redatto un documento nel quale saranno presenti tali statistiche, in modo che quando il responsabile riterrà che la raccolta dati risulti sufficientemente si passerà alla tecnica di \inglese{inspection}, tecnica che richiede un minor costo temporale e di conseguenza economico.\\
...
\subsection{Metriche per la verifica del codice}\label{sec:metrics}
...
\begin{itemize}
	...
	\item spronare i membri del team ad autogiudicarsi sul il proprio operato, scontrandosi (per mezzo di critiche costruttive) con il giudizio altrui;
...
\subsection{Autovalutazione di qualità dei processi}

\subsection{Autovalutazione dei ruoli}

\subsection{Autovalutazione degli strumenti}
