...
In fase di codifica della parte JavaScript i programmatori dovranno fare in modo che il codice prodotto sia mantenuto in file con estensione \texttt{.js} separati rispetto alla parte \textit{php} e \textit{html} e che in tutti i documenti \textit{html} che utilizzeranno tale codice sia inserito al loro interno, nel tag \inglese{head}, il riferimento al file JavaScript esterno.
...
Per i test di unità al contrario la verifica dovrà basarsi sulle specifiche relative test di unità e di integrazione prodotte dalle attività di progettazione architetturale e di dettaglio.

Tali modalità sono specificate in maniera esauriente nel documento \textit{piano\_di\_qualifica.3.0.pdf} allegato. Verranno svolti tramite \textit{Eclispe} tramite il \textit{framework} JUnit e QUnit descritti nella sezione relativa agli strumenti di verifica.
