...
% Verificatori
\newcommand{\verifiedBy}{Marco Schivo\\&Elena Zecchinato\\}
...
1.5 & Correzione errori rilevati dal verificatore & Diego Beraldin & Amministratore & 2013-01-17\\
1.4 & Validazione del documento & Elena Zecchinato & Verificatore & 2013-01-17\\
...
\item \textbf{Livello di dettaglio}: raggiungere un livello di dettaglio in modo tale da permettere la parallelizzazione della codifica e non lasciare decisioni ai programmatori. 
...
Per ogni design patterns utilizzato, si dovrà dare una breve descrizione indicando:
\begin{itemize}
\item una breve descrizione riguardante la struttura generale;
\item il motivo per cui è stato scelto e di conseguenza usato;
\item elenco dei contesti dove è stato utilizzato il design patterns.
\end{itemize}

\subsection{Convenzioni sui diagrammi}
Si dovrà utilizzare il linguaggio UML per definire:
\begin{itemize}
\item \textbf{Diagrammi di attività}: dovranno essere presenti per l'architettura generale. Questi diagrammi rappresenteranno un generico flusso d'esecuzione dell'intero applicativo. Nel caso non sia possibile inserire tutto in un solo diagramma, questo dovrà essere scomposto in sotto-diagrammi. Tale regola serve anche per ottenere una maggiore leggibilità e chiarezza.
\item \textbf{Diagrammi di sequenza}: possono essere presenti per l'architettura generale. Devono essere usati per descrivere una determinata sequenza di azioni in cui tutte le scelte sono state già effettuate. Questo significa che nel diagramma non compaiono scelte, né flussi alternativi.
\item \textbf{Diagrammi deli package}: dovranno essere presenti per la parte server implementata con Java. Servono per definire i package, ossia un raggruppamento di classi.
...
\subsection{Tracciamento}
Per il tracciamento componenti-requisiti e viceversa, si dovranno utilizzare due tabelle. La prima sarà composta da due colonne, una per il nome del componente e l'altra per i requisiti. La seconda tabella invece sarà composta da tre colonne, la prima per il codice del requisito, la seconda per la descrizione del requisito e la terza colonna per i componenti.
...
\item breve trattazione dei design patterns utilizzati seguendo le norme indicate nel paragrafo \ref{sec:design_patterns};
\item descrizione dettagliata dei componenti rilevati in fase di progettazione indicando il tipo, la funzione e l'obbiettivo;
\item diagrammi UML;
\item tracciamento componenti-requisiti, requisiti, componenti.
\end{itemize}
...
\begin{itemize}
 ...
 \item gli strumenti per sviluppatori integrati in \textbf{Google \underline{Chrome}} (\url{https://developers.google.com/chrome-developer-tools}) e, in particolare:
   ...
   \item la sezione Timeline che permette di quantificare i tempi necessari al caricamento e all'esecuzione degli script, nonché di tracciare l'utilizzo della memoria e forzare l'invocazione del \inglese{garbage collector};
...
\item Se è la prima volta che il documento viene verificato, il verificatore procederà con un metodo di tipo \underline{walkthrough}, come descritto in sezione \ref{analisi_statica}. Se invece il documento era già stato verificato e rimandato per via di errori, si adopererà un metodo di tipo \underline{inspection},come descritto in sezione \ref{analisi_statica}.
