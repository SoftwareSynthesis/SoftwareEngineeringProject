...
\item elenco dei contesti dove è stato utilizzato il design pattern.
\end{itemize}

\subsection{Convenzioni sui diagrammi}
Si dovrà utilizzare il linguaggio UML per definire:
\begin{itemize}
\item \textbf{Diagrammi di attività}: dovranno essere presenti per l'architettura generale. Questi diagrammi rappresenteranno un generico flusso d'esecuzione dell'intero applicativo e nel caso non sia possibile inserire tutto in un solo diagramma, questo dovrà essere scomposto in sotto-diagrammi. Tale regola ha lo scopo di ottenere una maggiore leggibilità e chiarezza.
\item \textbf{Diagrammi di sequenza}: possono essere presenti per l'architettura generale. Devono essere usati per descrivere una determinata sequenza di azioni in cui tutte le scelte sono state già effettuate. Questo significa che nel diagramma non compaiono né scelte, né flussi alternativi.
...
\subsection{Tracciamento}
Per il tracciamento componenti-requisiti (e viceversa), si utilizzeranno due tabelle: la prima sarà composta da due colonne, una per il nome del componente e l'altra per i requisiti, la seconda tabella invece sarà composta da tre colonne, la prima per il codice del requisito, la seconda per la descrizione del requisito e la terza colonna per i componenti.
