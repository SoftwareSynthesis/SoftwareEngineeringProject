...
\item Se è la prima volta che il documento viene verificato, il verificatore procederà con un metodo di tipo \textit{walkthrougth}, ossia analizzerà l'intero documento dall'inizio alla fine. Se invece il documento era già stato verificato e rimandato per via di errori, si adopererà un metodo di tipo \textit{inspection}, ossia analizzerà il documento solo dove sono state apportate le modifiche accertandosi che siano stati corretti gli errori rilevati e che non ne siano stati introdotti nuovi.
\item Il verificatore prenderà nota in modo ordinato di tutti gli errori riscontrati e procederà con la creazione dei tickets relativi che saranno rivolti al redattore del documento come descritto nella sezione \ref{sec:ciclo_vita_ticket} ``Ciclo di vita di un Ticket''.
\end{enumerate}
