...
\subsubsection*{Funzione}
Estensione di \texttt{org.apache.catalina.websocket.WebSocketServlet} ed implementazione di \inglese{servlet}. ControllerManager rappresenta l'implementazione del pattern \textit{front controller}. Esso fornisce un unico punto d'accesso al sistema. Tale implementazione avviene definendo la classe come una \inglese{servlet} il cui compito sarà quello di delegare al controller corretto la richiesta inoltrata dal client.
...
\subsubsection*{Funzione}
\textit{Controller} usata per modificare i dati utente. Nello specifico il client potrà operare una modifica ai campi \texttt{nome} e \texttt{cognome}. Lo sviluppatore deve ricordarsi che i dati di autenticazione risiedono nella session con il quale il client sta interagendo con il \textit{front controller}.
...
Il flusso principale del metodo prosegue definendo un blocco try-catch-finaly. Il catch definito cattura un eccezione generica di tipo Exception e si occupa semplicemente di impostare result a null (si sottolinea che result è la stringa da ritornare nel Writer per informare il chiamante sul risultato della chiamata a metodo). Inoltre è definito un bloco finaly che si occupa di inizializzare writer a partire da response.getWriter(). Quindi termina scrivendo nel writer il contenuto della stringa result.

Tornando al blocco try, in esso seno definiti i seguenti step:

\begin{itemize}
	\item si impostano i dati relativi al nome, cognome, mail e immagine profilo, con quelli ottenibili da una chiamata a metodo getParameter(nomeParametro), richiamabile a partire dall'oggetto \texttt{request};
	...
	\item si esegue un controllo sul filePath della nuova immagine profilo. Se si verifica che il filePath è diverso da null, allora si prosegue con le operazioni necessarie per caricare nel server l'immagine dell'utente e impostando in un secondo momento il path dell'immagine con quella nuova (usare il metodo setPath() di user);
	\item si richiama update(user) a partire dall'oggetto dao. Si imposta result a \inglese{true}.
\end{itemize}
...
\begin{description}
	...
	\begin{itemize}
		...
		\item \texttt{Restituzione risultato}: dopo aver elaborato i dati, il \textit{controller} dovrà scrivere nel \texttt{PrintWriter} associato alla request:
			\begin{itemize}
...
\begin{description}
	...
	\item{\method{\# doAction(request: HttpServletRequest, response: HttpServletResponse): void}}\\
	Metodo usato per accedere alla funzionalità del \textit{controller}. Il metodo può essere pensato come suddiviso in due parti logiche: la craezione delle variabili locali, e il blocco try-catch di esecuzione delle istruzioni di aggiunta al db. Nella prima parte il metodo definisce le seguenti variabili (NOTA: il nome usato non può essere cambiato a piacere, ma deve essere quello qui prestabilito):
...
Dopo questa fase di inizializzazione, il metodo definisce un blocco try-catch. Il catch definito cattura un eccezione generica di tipo Exception e si occupa semplicemente di impostare result a null (si sottolinea che result è la stringa da ritornare nel Writer per informare il chiamante sul risultato della chiamata a metodo). Inoltre è definito un bloco finaly che si occupa di inizializzare writer a partire da response.getWriter(). Quindi termina scrivendo nel writer il contenuto della stringa result.

Tornando al blocco try, in esso si evidenziano i seguenti step:
...
\begin{itemize}
	...
	\item si inserisce la chimata cosi creata attraverso un istruzione dao.insert(call);
	\item si inserisce la prima CallList associata, definendo una nuova CallList da memorizzare in userCall. Quindi si imposta il suo contenuto con call e user e si termina questo step aggiungendo userCall al contenuto del db (usare sempre una chiamata a metodo dao.insert(userCall);
	\item si inserisce la seconda CallList. La procedura è identica a quella dello step precedente, con la differenza che  al posto di user si usa other per inpostare il campo utente della chiamata;
	\item il blocco try termina impostando result a \inglese{true}.
\end{itemize}
...
Il metodo inizia caricando i parametri ricevuti mediante \texttt{HttpServletRequest}. Una chiamata a tale metodo corrisponde ad una richiesta di registrazione al sistema e si attua impostando i dati di registrazione:
	...
	\begin{itemize}
		...
		\item picturePath;
	\end{itemize}
	Il passo successivo (sempre dentro il blocco try) consiste nel creare un istanza di \classname{dao.DataPersistanceManager} userDAO, e nell'eseguire un operazione di userDAO.\method{insert()} passando come parametro user (l'utente precedentemente creato). Il blocco try termina impostando result a ``\inglese{true}''.
...
\subsubsection*{Funzione}
\inglese{Conroller} da richiamare per gestire la visualizzazione della domanda segreta per il recupero della password.
...
Il metodo inizia creando un istanza di \classname{DataPersistanceManager} dao. Quindi si caricano i parametri ricevuti mediante \texttt{HttpServletRequest} e si usano per la creazione di un \classname{IUserData}. Ciò significa che il metodo dovrà creare una stringa per memorizzare la mail e inizializzarla con una chiamata getParameter(''username'') da eseguire a partire dal parametro d'ingresso request. Successivamente si inizializza una variabile \classname{IUserData} denominata user (ciò avviene con un istruzione \verb|dao.getUserData(mail)|). Quindi si definisce una clausola condizionale if else. Il ramo if è caratterizzato dalla condizione user != null. Nel caso il metodo definisce al suo interno le istruzioni per restituire la domanda segreta. Le operazioni qui sono:
\begin{itemize}
	\item si definisce uno String question e lo si inizializza con \verb|user.getQuestion()|;
	\item si definisce un PrintWriter writer;
	\item si scrive nel writer la domanda question.
\end{itemize}

Nel caso la condizione dell'if risulti falsa, il metodo definisce un ramo else con le istruzioni necessaria per scrivere nel writer la stringa ''null''.
\end{description}

\classsection{AnswerController}

\subsubsection*{Funzione}
\inglese{Conroller} che ha lo scopo di verificare la risposta alla domanda segreta per il recupero della password.
...
A questo punto il metodo definisce un costrutto condizionale if-else. L'if viene definito con la condizione answer != null. Nel caso il metodo procede definendo un blocco try catch. Nel blocco try, si procede sovrascrivendo answer con la criptazione di answer stesso. Tale operazione necessita dell'uso di strategy.encode(answer). Ora se answer è uguale alla risposta presente nell'oggetto user, allora si procede decriptando la password (usare strategy.decode(user.getPassword());) e definendo la seguente istruzione:\\

\verb|boolean success = this.sendMail("mytalk@softwaresynthesis.org",|\\
\verb|user.getMail(), "Recupero password", password);|\\

Se success contiene il valore \inglese{true} allora il metodo imposta result a \inglese{true}, altrimenti lo imposta a null. Se in seguito al controllo precedente si osserva che answer è uguale a null, allora il metodo imposta result a null. Analogamente si fa nel ramo catch del blocco try-catch creato in precedenza. Il metodo termina scrivendo in write il risultato result.
...
All'interno di questi blocchi viene eseguita un operazione di return \inglese{true}.

Il blocco try invece procede come segue:
...
\begin{itemize}
	...
	\item infine si impostano le procedure per l'invio della mail. Data la sensisbilità di tale fase, è importante che il programmatore riporti l'utilizzo dell'oggetto \classname{Trasport} cosi come qui descritto:\\
...
\begin{description}
  ...
  \end{verbatim}
   ...
   \item{\memberdata{-- thisPanel: HTMLElement}}\\
  Riferimento all'elemento, da intendersi come nodo DOM, che rappresenta il pannelo controllato da questo \inglese{presenter} che nello specifico è rappresentato da un \verb'<div>' con l'identificativo \verb'AddressBookPanel'.
...
\item{\method{+ getGroupsWhereContactId(contact:Object): Object}}\\
...
Poiché il server ritorna la stringa \verb'"true"' se l'operazione va a buon fine, è sufficiente che il metodo restituisca \verb'true' se il risultato di \verb'JSON.parse(request.responseText)' è valutato positivamente e \verb'false' in caso contrario. Inoltre, nel caso in cui il gruppo sia stato effettivamente rimosso, dovrà essere richiamato il metodo \method{setup()} del \inglese{presenter}.
...
Poiché il server ritorna la stringa \verb'"true"' se l'operazione va a buon fine, è sufficiente che il metodo restituisca \verb'true' se il risultato di \verb'JSON.parse(request.responseText)' è valutato positivamente e \verb'false' in caso contrario. Inoltre, nel caso in cui il contatto sia stato effettivamente rimosso dalla rubrica, dovrà essere richiamato il \method{setup()} del \inglese{presenter}.
...
\item{\method{+ onLoadedView(presenter: String, view: HTMLElement): void}}\\
...
La gestione dell'evento dovrà differenziarsi a seconda del fatto che si tratti di una vista di primo o di secondo livello o non appartenga a nessuno dei due casi, in base al valore del primo parametro che identifica il \inglese{presenter} destinatario.

Nel caso di una vista di primo livello, dovrà essere richiamato il metodo \method{initialize} messo a disposizione dalla vista stessa, passando l'elemento HTML ricevuto come secondo parametro per la visualizzazione e l'inizializzazione della vista.
...
Infine, se il primo parametro non corrisponde ad alcuno dei \inglese{presenter} di primo o di secondo livello, allora l'evento \verb'loadedView' e stato sollevato al fine di visualizzare un pannello temporaneo il cui sotto-albero ha come radice l'elemento HTML ricevuto come secondo parametro.
...
\item{\method{+ onRemoveAllPanels(): void}}\\
...
% Questo è morto!
%\item{\method{+ onRemoveAnswerBox(): void}}\\
%Questo gestore di eventi ha lo scopo di rimuovere il pannello temporaneo che permette all'utente di rispondere a una chiamata in ingresso (in caso di accettazione o di rifiuto della stessa). A tal fine dovranno essere recuperati i riferimenti ai due elementi \verb'<div>' presenti nel documento con identificativi, rispettivamente, uguali a \verb'overlayAnswerBox' e \verb'answerBox', quindi rimuoverli dai figli dell'elemento \verb'<body>'.
...
Inizialmente, si inizializza la variabile \memberdata{pc} che è di tipo RTCPeerConnection, oggetto reso disponibile dalle librerie di WebRTC, successivamente si definiscono i comportamenti associati agli eventi di RTCPeerConnection, ossia:
  \begin{itemize}
    \item onicecandidate: evento scatenato quando un nuovo \inglese{peer} si ``candida'' per poter chiamare, viene quindi inviata la propria ``descrizione'' all'altro peer.
    ...
    \item onremovestream: evento scatenato quando viene rimosso uno \textit{stream} nell'oggetto RTCPeerConnection. In questa situazione, colui che effettua questa operazione vuole terminare la chiamata e deve quindi re-inviare la propria descrizione (attraverso il metodo \method{gotDescription()} in modo tale che l'altro \inglese{peer} possa aggiornare la descrizione remota. Vengono richiamati alla fine i metodi che fermano la visualizzazione delle statistiche di chiamata e chiudono il canale di comunicazione tramite la chiamata del metodo \verb'close()' dell'oggetto \memberdata{pc}.
  \end{itemize}

  \item{\method{+ connect(): void}}\\
  Metodo utilizzato per la creazione della connessione con il \textit{server}. Viene inizializzata la variabile \memberdata{websocket} richiamando il costruttore con un parametro (URL del sottotipo di \verb'WebSocketServlet' in ascolto sul server). Successivamente viene specificato il comportamento della WebSocket per determinati eventi quali:
  \begin{itemize}
    ...
    \begin{itemize}
      \item tipo 2: riceve e gestisce una richiesta di chiamata. Imposta quindi con il metodo della libreria di WebRTC \verb'setRemoteDescription' la descrizione del \inglese{peer} chiamante (secondo elemento dell'array ricevuto) e la comunicazione può iniziare.
  ...
  Metodo richiamato per terminare una chiamata, a cascata viene richiamato il metodo \verb'removeStream(localstream)' e inviata la nuova descrizione al \inglese{peer} remoto.
  
  \item{\method{+ handleCall(caller: Object, onlyAudio: Boolean)}}\\
  ...
  \item{\method{+ isPCDefined(): Boolean}}\\
  Questo metodo ha il compito di restituire \verb'true' se è attiva una RTCPeerConnection (in base al valore del campo dati \memberdata{pc}) e \verb'false' altrimenti.
...
\begin{description}
  ...
  \begin{itemize}
    ...
    \item[--] alla pressione del pulsante per avviare una chat testuale con il contatto dovrà essere sollevato un evento con l'identificativo \verb'chatStarted' dopo averne impostato la proprietà \verb'contact' col valore di \memberdata{currentContact};
...
\begin{description}
  ...
  \item{\method{+ correctAnswer(): void}}\\
	Metodo usato per modificare il nodo DOM salvato nella variabile \memberdata{thisPanel} per comunicare all'utente che la risposta da lui inserita per il recupero password è corretta. 
