...
\item[] \textit{specifica\_tecnica.3.0.pdf} allegato
\end{itemize}
...
\section{Specifica sotto-architettura \texttt{server}}\label{sec:serverarchitecture}

\subsection{Package \texttt{org.softwaresynthesis.mytalk.server}}\label{sec:server}

%\begin{center}
%\begin{figure}[H]
 % \includegraphics[width=\textwidth]{DDPServer}
...
\begin{center}
\begin{figure}[H]
  \includegraphics[width=.8\textwidth]{DDPAbookController}
...
\begin{description}
  ...
  Attributo necessario al caricamento delle credenziali di accesso per la corretta esecuzione della fase di autenticazione.
  \item{\memberdata{-- strategy: ISecurityStrategy}}\\Attributo che definisce l'algoritmo di criptazione e decriptazione dei dati.
\end{description}
...
\begin{center}
\begin{figure}[H]
  \includegraphics[width=0.7\textwidth]{DDPConnection}
...
\begin{description}
  ...
  Attributo usato per memorizzare lo stato dell'utente ``proprietario'' del \classname{PushInbound}, dove \texttt{State} è un tipo enumerativo i cui valori possibili sono  ``Available'' e ``Occupeied''.
\end{description}
...
\begin{description}
	...
	\item{\method{+ calculateScore(character: String): Number}}\\
