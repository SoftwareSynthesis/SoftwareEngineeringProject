...
\begin{description}
	\item{\method{\# doAction(request HttpServletRequest, response HttpServletResponse): void}}\\	
	Metodo usato per eseguire la richiesta di modifica dei dati di un nuovo client, registrato all'interno del database. Il flusso principale inizia  con la creazione di un oggetto \classname{dao.DataPersistanceManager} avente nome userDAO. Quindi viene aperto un blocco try catch in cui si salva in un istanza di tipo \texttt{HttpSession}, l'oggetto ritornato da una chiamata \verb|request.getSession(false)|. I passi successivi sono (nell'ordine):
	\begin{itemize}
		
		\item si crea un istanza di tipo \classname{IUserData} a partire da una chiamata a metodo:\\
		\verb|session.getAttribute("user")|;
		
		\item si crea un istanza di tipo \classname{IUserData} che conterrà ``l'utente'' i cui dati dovranno essere modificati. L'oggetto dovrà essere istanziato in seguito ad una chiamata:\\
		\verb|userDAO.getByID(contactId)|;
		
		\item quindi si procede modificando i campi di user: name e surname. L'istruzione da utilizzare è:\\
		\verb|name = request.getParameter("name");|\\
		\verb|surname = request.getParameter("surname");|
	\end{itemize}
	Quindi se il contatto (i cui dati dovranno essere modificati) è stato correttamente istanziato (contatto != null) allora il flusso principale procede creando e impostando un istanza di \classname{UserData}. Il metodo termina ``scrivendo'' \inglese{true} all'interno di un istanza di \texttt{PrintWriter} creata a partire da \texttt{response.getWriter()} ed eseguendo una chiamata di aggiornamento del database (uso delle classe \classname{dao.DataPersistanceManager}.
	
	Se invece si è osservato che l'oggetto contenente il contatto da "aggiornare", ha valore uguale a null, allora il metodo termina scrivendo \inglese{false} all'intero della dello stesso \texttt{PrintWriter} già citato.
\end{description}
...
\classsection{AddCallsController}
