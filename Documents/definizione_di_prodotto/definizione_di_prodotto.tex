...
% Qui dovrà andarci COMMUNICATIONPRESENTER
...
Questo gestore di eventi viene richiamato quando si verifica un evento con stringa identificativa uguale a \verb'loadedView'. La gestione dell'evento dovrà differenziarsi a seconda del fatto che si tratti di una vista di primo o di secondo livello, in base al valore del primo parametro che identifica il \inglese{presenter} destinatario.
...
\item{\method{+ onShowAnswerBox(content: HTMLElement): void}}\\
Questo gestore di eventi ha il compito di far visualizzare un pannello temporaneo il cui sotto-albero ha come radice l'elemento HTML ricevuto come parametro, per permettere all'utente di rispondere a una chiamata in ingresso.

A tale scopo dovrà essere creato un \verb'<div>' con l'identificativo \verb'overlayAnswerBox' che dovrà essere aggiunto come primo figlio dell'elemento \verb'<body>' e un secondo \verb'<div>' con identificativo \verb'answerBox' che dovrà essere aggiunto immediatamente dopo l'\inglese{overlay}. Tutti gli altri eventuali figli del \verb'<body>' dovranno essere posizionati in successivamente nell'albero DOM con un'istruzione del tipo \verb'body.insertBefore(overlay, bodyFirstChild)'.
...
%***********************************************************************
%*                QUESTO È IL CONFINE FRA IL BENE E IL MALE            *
%***********************************************************************
% TODO IBI ERIT FLETUS ET STRIDOR DENTIUM
...
\begin{description}
  ...
  \begin{itemize}
    ...
    \item[--] alla pressione del pulsante per avviare una comunicazione audio con il contatto dovrà essere sollevato un evento con l'identificativo \verb'call' dopo averne impostato la proprietà \verb'contact' col valore di \memberdata{currentContact} e la proprietà \verb'isVideo' a \verb'false';
    \item[--] alla pressione del pulsante per avviare una comunicazione audio/video con il contatto dovrà essere sollevato un evento con l'identificativo \verb'call' dopo averne impostato la proprietà \verb'contact' col valore di \memberdata{currentContact} e la proprietà \verb'isVideo' a \verb'true';
...
%Il metodo, che riceve come parametro un contatto (\textit{contact}, ha il compito di recuperare tutti i gruppi a cui il contatto appartiene e visualizzare nel \textit{div} opportuno una \textit{label} per ogni gruppo trovato indicandone il nome.
