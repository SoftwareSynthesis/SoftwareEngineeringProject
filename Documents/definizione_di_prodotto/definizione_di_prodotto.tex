...
\begin{description}
	...
	\begin{itemize}
		...
		\item \texttt{Elaborazione dati}: il metodo crea (a partire dai parametri estratti dalla request) una nuova chiamata \classname{Call} da aggiungere al database. Si ricorda che i dati di una chiamata (o meglio di un oggetto che implementa l'interfaccia \classname{ICall}) sono espressi nel formato:\\
		
		\verb|{name:"NOME_UTENTE",start="DATA_INIZIO",end="DATA_FINE"}|\\
		
		Si consideri che la dove vi è un nome in maiuscolo, vi dovrà essere il valore analogo con cui impostare il transfer object da aggiungere nel database.
