...
\begin{description}
    ...
    \item \texttt{String} se l'attributo è destinato a contenere esclusivamente valori di tipo \textit{String} che sarà segnato nel documento come:
			...
			\item array, se l'attributo è un \inglese{array}, che in JavaScript non chiede di essere definito per tipo di valori contenibili, che nel documento sarà segnalato con una coppia di quadre, ad esempio nel caso di un array di stringhe:
...
\begin{itemize}
  ...
  \begin{verbatim}
    this.attributo = new Costruttore();
  \end{verbatim}
...
\begin{verbatim}
  this.nome_metodo = function(arg0, arg1, ...) {
    corpo_del_metodo
  }
\end{verbatim}
...
\begin{verbatim}
  function nome_metodo(arg0, arg1, ...) {
  corpo_del_metodo
  }
\end{verbatim}
...
% Tutto questo è stato trasferito alla vita o è diventato obsoleto
% 
...
% TODO quanto segue è obsoleto e/o è stato trasferito alla vista
%
...
\begin{description}
  ...
  Tale metodo ha il compito di eliminare il pannello dall'interfaccia grafica visualizzata. A tal fine, deve essere rimosso il nodo che corrisponde al pannello nell'albero DOM che corrisponde alla UI\@.
...
Infine, il metodo restituisce il nodo DOM radice della nuova porzione di interfaccia grafica creata, vale a dire il \verb'<fieldset>'.
	...
	\item{\method{+ initialize(HTMLElement): void}}\\
	Il metodo ha il compito di inizializzare il pannello di login dopo che è stato caricato il relativo \inglese{template} per la creazione di tutti i \inglese{widget} grafici che sono contenuti al suo interno.
...
\begin{itemize}
  \item[] Un elemento \verb'<fieldSet>' con id uguale a \verb'loginForm' che contiene:
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \begin{itemize}
        \item[-] un elemento \verb'<button>' per effettuare il login;
        \item[-] un elemento \verb'<button>' per effettuare la registrazione al sistema;
        \item[-] un elemento \verb'<button>' per recuperare i dati dimenticati.
      \end{itemize}
    \end{itemize}
  \end{itemize}
\end{itemize}
