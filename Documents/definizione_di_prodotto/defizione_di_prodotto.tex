...
Poiché l'iter presentato può causare il lancio di un'eccezione, il codice dovrà essere gestito in un blocco \inglese{try-catch}. Nel caso incorra il lancio tale evento, il flusso di processo dovrà ristabilire la situazione iniziale richiamando il metodo \texttt{rollback()} dell'istanza di tipo \texttt{org.hibernate.Transaction} creata al punto 4 dell'iter. In ogni caso il metodo deve terminare chiudendo la sessione aperta al punto 2.
	...
	Metodo usato per eseguire un interrogazione al database atta ad ottenere la lista degli utenti aventi un determinato nome e cognome, riceve come parametro il nome e il cognome da ricercare. Il metodo dovrà eseguire nell'ordine le seguenti operazioni:
		\begin{itemize}
			\item[1)] ottenere dalla classe \classname{HibernateUtil} l'istanza tramite il metodo statico \method{getIstance()};
			\item[2)] ottenere dall'istanza di tipo \classname{HibernateUtil}(ottenuta al punto precedente), un oggetto di tipo \texttt{org.hibernate.SessionFactory}, tramite il metodo \method{getSessionFactory()};
			\item[3)] aprire la sessione ottenuta al punto precedente;
...
Metodo che recupera i contatti e i gruppi della propria rubrica dal server usando la tecnologia AJAX. Il metodo crea una variabile contenente un istanza \texttt{XMLHttpRequest()}. Quindi con metodo \inglese{POST} inoltra la richiesta alla \inglese{servlet} richiamandola al URL salvato nell'attributo \memberdata{urlServlet}. Quindi elabora i dati ottenuti mediante il metodo \texttt{JSON.parse()} e li salva negli array \memberdata{contacts} e \memberdata{groups}.
...
Metodo che testa quanto ricevuto dal server e, in caso di login avvenuto correttamente reindirizza il browser nella pagina finale dopo aver salvato i dati dell'utente. Il metodo inizia salvando, in una variabile avente nome user, il contenuto di data trattato attraverso una chiamata \texttt{JSON.parse(data)}. Quindi se il contenuto di user non è nullo si procede richiamando il \texttt{communicationcenter} e impostando il valore di \texttt{communicationcenter.my} a user. Il metodo termina richiamando  il metodo \method{hide()} e avviando la costruzione della UI mediante una chiamata \method{medietor.buildUI()}.
	...
	\item{\method{+ hasAnsweredCorrectly(username: String, answer: String): boolean}}\\
Metodo utilizzato per verificare se la risposta alla domanda segreta fornita dall'utente corrisponde a quella impostata in fase di registrazione al sistema. Il metodo riceve 2 parametri d'ingresso: lo username dell'utente e la risposta (answer) da inviare al server. Come primo passo viene creata un istanza di \texttt{XMLHttpRequest} salvata in una variabile avente nome request.

La richiesta al server dovrà avere carattere sincrono e, al ritorno, il testo della risposta dovrà essere recuperato sotto forma di stringa, convertito in un valore JavaScript (\texttt{true} o \texttt{false}) e quindi restituito al chiamante.
...
il messaggio sopracitato è temporaneo e dovrà essere rimosso allo scadere di un timeout di durata 2000 millisecondi (usare la funzione \texttt{window.setTimeout(nome\_funzione))}.
	...
	Metodo usato per costruire il form per il recupero della password utente. Il metodo inizia costruendo il form con il valore ottenuto mediante una chiamata \verb|document.createElement("fieldset")|. Quindi a partire dalla variabile in cui ha memorizzato il dato ritornato con il metodo precedente, impostando l'attributo ``id'' con ``passwordretrieval''. Definisce quindi una label con la domanda (utilizzare \verb|document.createElement("label")| e imposta l'attributo ``for'' con ``inputanswer''). Il metodo procede impostando il campo d'immissione della domanda. Successivamente si impostano i seguenti attributi:
	\begin{itemize}
	...
	Associare all'evento click del bottone, una funzione che richiami \method{sendAnswer()} passando come attributi \memberdata{username} e  il valore contenuto nell'elemento \texttt{inputAnswer}.
	
	In ultima si usa \texttt{appendChild} per agganciare al nodo DOM la label e \texttt{inputAnswer}. Il metodo termina ritornando il nodo DOM.
	...
	Metodo che esegue il login inviando al server i dati di autenticazione. Il metodo riceve come parametro d'ingresso un array associativo contenente ``username'' e ``password''.
	
	Il flusso principale inizia verificando il che il contenuto di \texttt{data.username} e di \texttt{data.password} non sia vuoto. Quindi invia una richiesta AJAX al server (come sempre in questo caso è d'obbligo usare \verb|var request = new XMLHttpRequest();|).
	Il programmatore dovrà associare all'evento \texttt{onreadystatechange} una funzione che si occupa di richiamare il metodo \method{testCredentials()}. Il passo successivo consiste ``nell'aprire'' la richiesta request con un istruzione open() che riceve come parametro l'url della \inglese{servlet} associata (valore presente in \memberdata{servletURL}). A questo punto di usa un istruzione \texttt{setRequestHeader()} passando i seguenti parametri:
	\begin{itemize}
		\item[•] ``content-type'';
		\item[•] ``application/x-www-form-urlencoded'';
	\end{itemize}
	In ultima va creata la querystring da inviare (tramite istruzione request.send()). Tale query deve essere obbligatoriamente impostata a:\\
...
\begin{description}
	...
	\item{\method{+ getSurname(): String}}\\
	Metodo che estrae dal form il valore del cognome del nuovo utente e lo restituisce.
	
	\item{\method{+ getName(): String}}\\
	Metodo che estrae dal form il valore del nome del nuovo utente e lo restituisce.
	
	\item{\method{+ getAnswer(): String}}\\
	Metodo che estrae dal form la risposta alla domanda segreta associata al nuovo utente e lo restituisce.	
		
	\item{\method{+ getQuestion(): String}}\\
	Metodo che estrae dal form la domanda segreta associata al nuovo utente e lo restituisce.
	
	\item{\method{+ getPassword(): String}}\\
	Metodo che estrae dal form la password associata al nuovo utente e la restituisce.
	
	\item{\method{+ getUsername(): String}}\\
	Metodo che estrae dal form lo username del nuovo utente (per username si intende l'indirizzo email dell'utente). Si osservi che il metodo dovrà verificare che la mail restituita sia ben formattata secondo l'espressione regolare:\\
	...
	\item{\method{+ getPicturePath(): String}}\\
	Metodo che estrae dal form il percorso dell'immagine del nuovo profilo utente e lo restituisce.
	
	\item{\method{+ register(userData: Arrey()): String}}\\
	Metodo che Invia i dati ricevuti alla \inglese{servlet} per la creazione di un nuovo account utente. Il metodo inizia creando un istanza (request) di \texttt{XMLHttpRequest} usata per comunicare con la \inglese{servlet}. Quindi il metodo abilita request ad inviare un richiesta sincrona al server, usando l'istruzione:\\
	
	\verb|request.open("POST", servletURL, false)|\\
	
	Il metodo procede definendo la stringa da usare come query per da inviare alla \inglese{servlet}. Tale querystring dovrà contenere:\\
	...
	\item{\method{+ initialize(): void}}\\
	Metodo che si occupa di inizializzare la form di registrazione con la creazione di tutti i \inglese{widget} grafici che sono contenuti al suo interno. Nello il metodo esegue i seguenti passi:
	\begin{itemize}
		\item[•] creazione dell'elemento form;
		\item[•] creazione dell'elemento <ul> contenuto nel form;		
		\item[•] creazione dell'item per lo username, definito mediante una label riportante la frase ``Indirizzo email: '', e di un campo input di tipo email per l'inserimento della email dell'utente;
		\item[•] costruisce il list item con la label e l'input definiti al punto precedente, usando il metodo \texttt{appendChild()};
		\item[•] crea l'item per la password, definendo una label riportante la frase ``Password: '' e un oggetto input di tipi password;			
		\item[•] costruisce il list item con la label e l'input definiti al punto precedente, usando il metodo \texttt{appendChild()};
		\item[•] creazione dell'item per la domanda segreta definito mediante una label avente valore ``Domanda segreta: '' e un oggetto input di tipo text;
		\item[•] costruisce il list item con la label e l'input definiti al punto precedente, usando il metodo \texttt{appendChild()};
		\item[•] creazione dell'item per la risposta alla domanda segreta, definito mediante una label avente valore ``Risposta: '' e un oggetto input di tipo text;
		\item[•] costruisce il list item con la label e l'input definiti al punto precedente, usando il metodo \texttt{appendChild()};
		\item[•] creazione dell'item per il nome, definito mediante una label avente valore ``Nome: '' e un oggetto input di tipo text;
		\item[•] costruisce il list item con la label e l'input definiti al punto precedente, usando il metodo \texttt{appendChild()};
		\item[•] creazione dell'item per il cognome, definito mediante una label avente valore ``Cognome: '' e un oggetto input di tipo text;
		\item[•] costruisce il list item con la label e l'input definiti al punto precedente, usando il metodo \texttt{appendChild()};
		\item[•] creazione dell'item per inserire l'immagine utente, definito mediante una label avente valore ``Immagine del profilo: '' e un oggetto input di tipo text;
		\item[•] costruisce il list item con la label e l'input definiti al punto precedente, usando il metodo \texttt{appendChild()};
