...
%TODO da completare doLogin, doLogout

\subsubsection*{Funzione}
\inglese{Servlet} per da richiamare per effettuare la login e la corrispettiva autenticazione dell'utente.

\subsubsection*{Relazioni d'uso}
\begin{itemize}
	\item \texttt{java.io.IOException}
	\item \texttt{java.io.PrintWriter}
	\item \texttt{javax.security.auth.login.LoginContext}
	\item \texttt{javax.servlet.ServletException}
	\item \texttt{javax.servlet.http.HttpServlet}
	\item \texttt{javax.servlet.http.HttpServletRequest}
	\item \texttt{javax.servlet.http.HttpServletResponse}
	\item \texttt{javax.servlet.http.HttpSession}
	\item \classname{abook.IUserData}
	\item \classname{dao.UserDataDAO}
\end{itemize}
...
\subsubsection*{Funzione}
Classe per la definizione di un apposito canale di comunicazione client-server. La classe è un'estensione di \texttt{org.apache.catalina.websocket.MessageInbound}, che verrà usata poi in \classname{ChannelServlet}.

\subsubsection*{Relazioni d'uso}
\begin{itemize}
	\item \texttt{java.io.IOException}:
	\item \texttt{java.nio.ByteBuffer}:
	\item \texttt{java.nio.CharBuffer}:
	\item \texttt{java.util.List}:
	\item \texttt{org.apache.catalina.websocket.MessageInbound}:
	\item \texttt{com.google.gson.*}:
...
\begin{description}
	...
	\begin{itemize}
		...
		\item[•]Se la richiesta inoltrata è del tipo 5: il metodo viene usato per modificare lo stato dell'utente, con il valore passato tramite messaggio. Dopo la ricezione del messaggio, il metodo si occupa di riscaricare l'oggetto \classname{abook.IUserData} dal database, quindi cambia il valore del campo \texttt{state} associato, con il valore ricevuto come parametro d'ingresso. Quindi procedo ricavando la lista degli ``amici'' dell'utente che ha cambiato stato, e comunico loro che è avvenuto un cambiamento di stato.
	\end{itemize}
\end{description}
...
\begin{description}
	...
	Metodo per inizializzare  ``AddressBookPannel'' e lo popolarlo con i contatti della rubrica. Il metodo modifica il DOM creando tre div: divSearch, divSort e divList. Quindi sequenzialmente ne crea il contenuto:
		\begin{itemize}
