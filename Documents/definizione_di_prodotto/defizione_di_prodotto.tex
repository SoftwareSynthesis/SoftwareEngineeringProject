...
Metodo richiamato da \method{displayContactList()} per visualizzare i contatti presenti all'array \memberdata{contacts} (passato come parametro). Il metodo crea le variabili contenenti i dati del contatto da attribuire al tag di markup \texttt{li}. Tali variabili sono quelle tipiche di un oggetto \classname{JSUserData} (nome, cognome, email, status e immagine).
...
\item{\method{+ blockUser(contact: Array()): boolean}}\\
...
\item{\method{+ unlockUser(contact: Array()): boolean}}\\
