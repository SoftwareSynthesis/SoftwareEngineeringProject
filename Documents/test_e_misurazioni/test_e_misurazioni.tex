...
1.0 & Approvazione documento& Andrea Rizzi & Responsabile & 2013-02-23\\
0.8 & correzione errori segnalati dal verificatore& Stefano Farronato & Verificatore & 2013-02-23\\
0.7 & verifica correttezza lessico ortografica e corrispondenza test con quanto descritto& Diego Beraldin & Verificatore & 2013-02-23\\
0.6 & aggiunti test su capitolo e test coverage& Stefano Farronato & Verificatore & 2013-02-22\\
0.5 & stesura capitoli sui test di sistema e di utilizzo, aggiunti test su capitolo 4.1& Stefano Farronato & Verificatore & 2013-02-22\\
0.4 & stesura capitolo relativo ai test sul \textit{presenter} & Stefano Farronato & Verificatore & 2013-02-21\\
0.3 & stesura capitolo relativo ai test sul \textit{model} & Stefano Farronato & Verificatore & 2013-02-21\\
0.2 & stesura introduzione con ``scopo dei test'' e preambolo su ``Metriche sul codice''& Stefano Farronato & Verificatore & 2013-02-20\\
...
AccountSettingPannelPresenter.js & XXXX & XXXX\\
AdressBookPanelPresenter.js & XXXX & XXXX\\
CallHistoryPanelPresenter.js & XXXX & XXXX\\
ComunicationPanelPresenter.js & XXXX & XXXX\\
ContactPanelPresenter.js & XXXX & XXXX\\
GroupPanelPresenter.js & XXXX & XXXX\\
LoginPanelPresenter.js & XXXX & XXXX\\
MainPanelPresenter.js & XXXX & XXXX\\
MessagePanelPresenter.js & XXXX & XXXX\\
PresenterMediator.js & XXXX & XXXX\\
RegisterPanelPresenter.js & XXXX & XXXX\\
SearchResultPanelPresenter.js & XXXX & XXXX\\
ToolsPanelPresenter.js & XXXX & XXXX\\
\midrule
...
\begin{itemize}
\item \textit{testInitialize()} crea la rubrica dell'utente, verificando che siano stati creati correttamente un \textit{H1} e quattro \textit{<div>} e che siano rispettivamente \textit{DA-SISTEMARE}, \textit{divFilter} , \textit{divSort} e \textit{divList}; infine verifica che i relativi figli di tali \textit{<div>} siano correttamente inizializzati;
...
\item \textit{testHide() } controlla che venga correttamente nascosto il \inglese{form} della rubrica;
\item \textit{testAddContact() } crea un contatto \inglese{stub} con dei parametri aleatori definiti dall'utente inserendolo nella lista contatti, viene assicurata la presenza di tale contatto confermando l'effettività della stessa.
\item \textit{testRemoveContact() } crea un contatto \inglese{stub} con dei parametri aleatori definiti dall'utente inserendolo nella lista contatti, una volta assicurata la presenza di tale contatto viene effettuata l'eliminazione confermando l'effettività della stessa.
\item \textit{testAddGroup() } crea uno \inglese{stub} con un gruppo inserendolo nella lista dei gruppi, viene assicurata la presenza di tale gruppo confermando l'effettività della stessa.
\item \textit{testDeleteGroup() } crea uno \inglese{stub} con dei parametri aleatori definiti dall'utente inserendolo nella lista dei gruppi, una volta assicurata la presenza di tale gruppo viene effettuata l'eliminazione confermando l'effettività della stessa.
\item \textit{testAddContactInGroup() } crea un contatto \inglese{stub} con dei parametri aleatori definiti dall'utente inserendolo nella lista contatti, successivamente crea uno \inglese{stub} con un gruppo inserendolo nella lista dei gruppi, una volta assicurata la presenza di entrambi, viene effettuato l'inserimento del contatto nel gruppo verificando l'effettività dell'azione.
\item \textit{testRemoveContactFromGroup() } %TODO DA DESCRIVERE
...
\textbf{Descrizione}: l'obbiettivo di questo test è verificare la corretta istanziazione del \textit{GroupPanel}, verificandone pertanto la corretta esecuzione della \underline{\inglese{form}}.\\
Tale verifica è composta da un solo test:
\begin{itemize}
\item \textit{testCreatePanel() }%TODO DA DESCRIVERE
\item \textit{testDisplayContactList() }%TODO DA DESCRIVERE
\item \textit{testAddListItem() } %TODO DA DESCRIVERE
\end{itemize}
\textbf{Risultato del test:} non sono stati rilevati errori.

\subsubsection{LoginPanelPresenterTest}
\textbf{Verifica l'oggetto:} \textit{LoginPanelPresenter}.\\
\textbf{Descrizione}: l'obbiettivo di questo test è verificare la corretta istanziazione del \textit{LoginPanel} popolato con dei dati coerenti rispetto allo scopo della \underline{\inglese{form}}.
