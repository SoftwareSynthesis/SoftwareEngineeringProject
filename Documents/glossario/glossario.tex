...
% Nome file
\newcommand{\docFileName}{glossario.4.0.pdf}
% Versione
\newcommand{\docVers}{4.0}
% Data creazione
\newcommand{\creationDate}{2012-12-17}
% Data ultima modifica
\newcommand{\modificationDate}{2013-05-11}
...
4.0 & Approvazione documento & Marco Schivo & Responsabile & 2013-05-11\\\\
3.1 & Aggiunta la voce ``mock'' & Diego Beraldin & Verificatore & 2013-05-11\\\\
% modifica alla voce Chrome aggiungendo la vesione
3.0 & Approvazione & Diego Beraldin & Responsabile & 2013-02-14\\\\
2.1 & Riformattazione del documento & Riccardo Tresoldi & Responsabile & 2013-02-14\\\\
...
\glossaryItem{black-box}{%
entità software o sottosistema visto dall'esterno esclusivamente in termini di comportamento, ovvero di risposte ai dati in ingresso, senza essere in possesso di alcuna conoscenza dello stato e dei meccanismi di funzionamento interni, in accordo con i principi di \textit{encapsulation} e \textit{information hiding}.
}

\glossaryItem{best practice}{%
prassi (dal greco $\pi\rho\hat{\alpha}\xi\iota\varsigma$, `azione') consolidata sulla base dell'esperienza comune in un determinato dominio e la cui validità è universalmente o in larga misura riconosciuta dagli esperti di tale dominio. In genere le best practices sono formalizzate in una serie di regole la cui applicazione offre buone garanzie di riuscita del prodotto finale.
}

\glossaryItem{brainstorm}{%
parola inglese che tradotta in italiano significa letteralmente ``tempesta di cervelli'', sta ad indicare una riunione, solitamente con un discreto numero di persone. Nella suddetta riunione ogni membro del gruppo propone liberamente idee anche apparentemente poco inerenti all'argomento.
}
...
\glossaryItem{Chrome}{%
browser moderno lanciato da Google Inc. nel 2008 e basato sul motore di rendering WebKit. Le versioni prese come riferimento dal team sono quelle a partire dalla 23.0.1271.97m.
}
...
\glossaryItem{CSS}{%
acronimo di \inglese{Cascading Style Sheets} (lett. `fogli di stile a cascata'), la tecnologia web che associata al linguaggio di HTML permette la definizione
...
\glossaryItem{DOM}{%
acronimo di \inglese{Document Object Model}, è lo standard ufficiale del W3C per la rappresentazione logica di documenti, neutrali sia per linguaggio che per piattaforma. Letteralmente si traduce con ''modello a oggetti del documento'' e nell'ambito del progetto si riferisce al documento HTML.
}
...
\glossaryItem{framework}{%
architettura precostituita che contiene l'implementazione di \underline{design pattern} in un determinato linguaggio di programmazione per facilitare la realizzazione di applicazioni da parte degli utenti programmatori: implica la cosiddetta \textit{inversione del controllo} in quanto la logica dell'utente di un framework viene inserita a posteriori ed è richiamata dalle componenti del framework.
}
\clearpage
...
\glossaryItem{JSP}{%
acronimo di \inglese{JavaServer Pages}, è una tecnologia di programmazione Web in \underline{Java} per lo sviluppo di applicazioni Web che forniscono contenuti dinamici in formato \underline{HTML} o XML. Si basa su un insieme di speciali tag con cui possono essere invocate funzioni predefinite o codice \underline{Java}. Rispetto ad altri linguaggi di scripting per la generazione di pagine Web dinamiche (e.g. PHP ed ASP), differisce per l'architettura interna del software che costituisce l'applicazione Web.
}
\clearpage
...
\glossaryItem{mock}{%
sostituto di un oggetto reale da utilizzarsi nel corso dell'attività di analisi dinamica per simulare la presenza di componenti esterni rispetto all'oggetto di test. Al pari degli \underline{stub}, anche i mock sono componenti passive che non implementano al loro interno logica complessa ma, a differenza di quanto avviene con gli stub, le verifiche eseguite sui mock non si riferiscono esclusivamente allo \textit{stato} mediante le consuete asserzioni, bensì interessano anche il \textit{comportamento} degli oggetti. A tal fine sono tenuti in considerazione anche aspetti dinamici noti solo a tempo di esecuzione del test quali, ad esempio, il numero delle invocazioni di metodo sul mock, il valore degli argomenti catturati, eccetera.
}
...
\glossaryItem{peer-to-peer}{%
architettura logica di una rete informatica in cui i nodi non sono gerarchizzati unicamente sotto forma di client o server fissi, ma come nodi equivalenti o paritari (in inglese \inglese{peer}). E' dunque un caso particolare dell'architettura logica di rete client-server.
}
...
\glossaryItem{step over}{%
definisce una procedura di debugging che prevede l'esecuzione di ogni singola riga di codice in maniera sequenziale, mostrando di volta in volta lo stato di avanzamento del programma.
}

\glossaryItem{step into}{%
definisce una procedura di debugging che prevede l'esecuzione di una chiamata a metodo, saltando alla prima riga di codice del metodo richiamato ed eseguendo lo step over del corpo del metodo. Alla fine del corpo del metodo, ritorna alla riga succesiva alla chiamata del medesimo.
}

\glossaryItem{step out}{%
definisce una procedura di debugging che prevede l'esecuzione automatica di una chiamata a metodo, e termina con l'istruzione successiva alla chiamata a metodo.
}

\glossaryItem{stub}{%
in fase di analisi dinamica, la componente passiva richiamata nel corso dell'attività di verifica da una parte attiva che costituisce l'oggetto del test. Si tratta di componenti di semplice implementazione in quanto hanno solo il ruolo di fornire in output risultati plausibili senza alcuna necessità di applicare gli algoritmi della ``reale'' logica di business.
}
\clearpage
...
\glossaryItem{WebSocket}{%
è una tecnologia web che fornisce canali di comunicazione full-duplex attraverso una singola connessione TCP\@. L'\underline{API} del WebSocket è stata standardizzata dal W3C e il protocollo WebSocket è stato standardizzato dall'IETF come RFC 6455. WebSocket è indipendente dal protocollo TCP, ed è disegnato per essere implementato sia lato browser che lato server, ma può essere utilizzato anche da qualsiasi applicazione client-server.
}

\glossaryItem{white-box}{%
entità software o sottosistema la cui struttura interna è totalmente osservabile anche dall'esterno ma, in genere, non alterabile direttamente. In fase di test la verifica strutturale prevede l'esame delle componenti in questo senso, il che rende l'attività più complessa per il livello di comprensione richiesto ma più efficace.
}
