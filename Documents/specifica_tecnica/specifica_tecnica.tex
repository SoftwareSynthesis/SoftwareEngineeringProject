...
\begin{description}
	...
	\item{\scshape\bfseries CS01 -- Gestione database}\\
In tale componente è la classe \texttt{dao.DataPersistanceManager} a definire dei metodi per la creazione di istanze di classi figlie di \texttt{dao.GetUtil} e \texttt{dao.ModifyUtil}.
...
\begin{description}
  ...
  \item{\scshape\bfseries CS01 -- Gestione database}\\
In tale componente la classe \texttt{dao.ModifyUtil} implementa il pattern \texttt{template method} definendo un metodo per l'esecuzione di un'operazione su \texttt{dao.DataPersistenceManager}, e lasciando l'implementazione dell'algoritmo alle classi figlie. Tali sono: \texttt{dao.InsertUtil}, \texttt{dao.UpdateUtil} e \texttt{dao.DeleteUtil}.

Anche la classe \texttt{dao.GetUtil} implementa lo stesso comportamento, lasciando alle classi figlie (\texttt{dao.GetCallUtil}, \texttt{dao.GetGroupUtil}, \texttt{dao.GetUserUtil}, \texttt{dao.NotInitialize}) il compito di definire l'algoritmo d'implementazione.
