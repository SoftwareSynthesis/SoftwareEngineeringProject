...
La componente utilizza un particolare canale di comunicazione che risponde al contratto dell' interfaccia \texttt{connection.Ichanel}. In tale interfaccia saranno definiti i metodi per l'inizializzazione di una chiamata client-client. L'implementazione fornita dal team si attua per mezzo di un riuso di codice. Nello specifico si utilizzera la classe \texttt{org.apache.catalina.websocket.MessageInbound} fornita dalle librerie di Apache. Tale classe sarà adattata alle firme dei metodi di \texttt{connection.Ichanel} per mezzo di un pattern adapter, implementato dalla classe \texttt{connection.ChanelAdapter}. 

\texttt{connection.CommunicationHandler} avrà quindi il compito di fornire al client richiedente (previa autenticazione), un istanza di \texttt{connection.ChanelAdapter}. Il client potrà usare tale istanza nel momento in cui avrà necessità di instaurare una connessione con un altro client. \texttt{connection.ChanelAdapter} infatti fungerà da canale di comunicazione con il server, fornendo una serire di metodi per avviare l'iter di connessione webRTC.
	\item{\scshape\bfseries Diagramma delle classi:}
	\begin{figure}[H]
\begin{center}
...
\end{figure}
	...
	\begin{itemize}[nolistsep, noitemsep]
	  ...
	  \item[-] \texttt{connection.IChanel}
	  \item[-] \texttt{org.apache.catalina.websocket.MessageInbound}
	  \item[-] \texttt{connection.ChanelAdapter}
	\end{itemize}
\end{description}
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
Interfaccia per la gestione delle richieste per ottenere le informazioni necessarie a un client per stabilire una comunicazione con altri client. In particolare le operazioni in essa dichiarate dovranno ritornare un istanza di un oggetto che implementa \texttt{IChanel}.
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
Implementazione che si fornisce dell'interfaccia \texttt{ICommunicationHandler}. Tale classe avrà il compito di autenticare i client che desiderano utilizzare l'applicativo, fornendo in risposta, un istanza di \texttt{IChanel}.
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
Interfaccia che definisce un canale di comunicazione client-server. Il canale proposto sarà usato al solo fine di processare una richiesta del client, per stabilire una connessione webRTC con un altro client.
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
Classe presente nelle librerie Apache. Viene utilizzata dal server e da un client per garantire una comunicazione reciproca basata sullìinvio e la ricezione di opportuni messaggi. Il team intende usarla e riadattarla con un pattern adapter, per implementare l'interfaccia \texttt{connection.IChanel}.
...
\item \texttt{ChanelAdapter}
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
Classe che risolve l'utilizzo del pattern adapter tra l'interfaccia \texttt{connection.IChanel} e la classe \texttt{org.apache.catalina.websocket.MessageInbound}.
...
Inoltre, allo scopo di gestire le comunicazioni in ingresso dal servern inerenti alla gestione della segreteria o della rubrica (cambiamenti di stato degli utenti), è stato predisposto il componente \textsf{Gestione remota}.

La logica di comunicazione fra client è incapsulata all'interno delle classi facenti parte del componente \textsf{Gestione comunicazione}. Queste ultime sfruttano un canale di comunicazione clinet-server per interagire con il server in merito alle richieste di comunicazione con altri client, andando a creare appositi canali di comunicazione che si avvalgono della tecnologia webRTC.
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
È il componente che rappresenta il vero cuore logico della comunicazione. In esso è presente la classe \texttt{kernel.ComunicationCenter}, il cui compito è quello di ottenere un oggetto di tipo \texttt{connection.IChanel} per comunicare con il server, e dovrà anche tener traccia delle comunicazioni aperte con i client usando la tecnologia webRTC.

Tali comunicazioni sono rappresentate dall'interfaccia IClientComunication che rappresenta il contratto di un oggetto utilizzabile come canale di comunicazione clinet-client. L'implementazione di tale interfaccia che utilizzerà il webRTC, prende il nome di \texttt{kernel.WebRTCComunication}. Tale classe viene istanziata da \texttt{kernel.ComunicationCenter} sulla base delle informazioni ottenute dall'istanza \texttt{connection.IChanel}. 

Nello specifico i due possibili iter sono:
\begin{itemize}
	\item il server riceve una richiesta da parte di un client A per parlare con noi. Il server usa l'istanza \texttt{connection.IChanel} per processare la richiesta. \texttt{kernel.ComunicationCenter} raccoglie le informazioni dal canale di comunicazione e le usa per istanziare un \texttt{kernel.WebRTCComunication}. Al termine della procedura, l'applicativo lato client potrà comunicare usando tale istanza.
	\item siamo noi a voler comunicare con un client A. Avanziamo la nostra richiesta al server, mediante \texttt{connection.IChanel}. Quindi man mano che otteniamo risposta, costruiamo l'oggetto WebRTCComunication per parlare con A.
\end{itemize}
  ...
  \end{figure}
	...
	\item{\scshape\bfseries Classi utilizzate:} 
	\begin{itemize}[noitemsep,nolistsep]
		\item[-] \texttt{kernel.ComunicationCenter}
		\item[-] \texttt{server.connection.IChanel}
		\item[-] \texttt{kernel.IClientComunication}
		\item[-] \texttt{kernel.WebRTCComunication}
	\end{itemize}  
\end{description}
...
Un altro esempio è la ricezione di un nuovo messaggio inserito nella propria segreteria.

%TODO immagine da aggiornare e riesportare
% 	\item{\scshape\bfseries Diagramma delle classi:}
% \begin{figure}[H]
%   \centering
...
\item \texttt{ComunicationCenter}
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
È la classe che ha il compito di gestire la comunicazione con il srver per quanto riguarda lo stabilire una comunicazione client-client di tipo webRTC.
...
\item \texttt{ICLientComunication}
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
È l'interfaccia che rappresenta un canale di comunicazione client-client.
...
\item \texttt{WebRTCComunication}
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
È la classe che implementa l'interfaccia \texttt{IClietComunication}. In particolare tale classe rappresenta una comunicazione client-client settata per funzionare con le webRTC.
...
Invece nel caso venga inoltrato un nuovo messaggio alla segreteria del cliente, tale evento dovrà essere notificato per mezzo di questa classa.
