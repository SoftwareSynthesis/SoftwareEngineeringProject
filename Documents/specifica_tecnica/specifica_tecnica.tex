...
\end{figure}
	...
	\begin{itemize}[nolistsep, noitemsep]
	  \item[-] \texttt{server.dao.CallDAO}
	   \item[-] \texttt{server.dao.CallListDAO}
...
Infine sono state predisposte 12 \inglese{servlet} per interagire con le classi qui descritte:
...
Infine, allo scopo di consentire ai client l'accesso alle funzionalità di questo componente, è stata prevista una \inglese{servlet} \texttt{call.servlet.DownloadCallHistoryServlet} che estende \texttt{javax.servlet.http.HttpServlet} il cui ruolo sarà descritto con maggiori dettagli nella sezione \vref{sec:cs07}.
...
Ai fini del \inglese{login} è utilizzata invece la \inglese{servlet} \texttt{authentication.servlet.LoginServlet}, che ha il compito di interfacciarsi con il sistema di autenticazione e, in particolare, con l'istanza del sottotipo di \texttt{javax.security.auth.spi.LoginModule} in uso per verificare le credenziali. Inoltre come già accennato sono presenti la \inglese{servlet} per il \inglese{logout} \texttt{authentication.servlet.LogoutServlet} e quella per la registrazione al sistema \texttt{authentication.servlet.RegisterServlet}.
...
Tali dipendenze, di fondamentale importanza per l'architettura, sono però riportate nei diagrammi della sezione \vref{sec:patternfacade} in cui è descritta con maggiori dettagli l'applicazione del \inglese{design pattern} Façade.
