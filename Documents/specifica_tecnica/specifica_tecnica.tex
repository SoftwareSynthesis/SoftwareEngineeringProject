...
La sotto-architettura \texttt{clientpresenter} corrisponde alla parte ``\inglese{presenter}'' dell'applicativo. Allo scopo di gestire le comunicazioni in ingresso dal server (ad esempio le notifiche dei cambiamenti di stato o le chiamate in ingresso) è stato predisposto un componente \textsf{Gestione remota}.
 
I componenti in quest'ultima sfruttano inoltre la comunicazione con il server, tramite opportune procedure di interrogazione attraverso il componente \textsf{Façade del server}, al fine di ottenere:
\begin{itemize}
...
Mentre i primi sono rappresentati dalle strutture del componente \textsf{Rappresentazione dati} e permangono internamente al \inglese{presenter} per essere disponibili a successive interrogazioni da parte della GUI senza coinvolgere il server, i secondi sono utilizzati da \textsf{Gestione comunicazione} per instaurare e mantenere le connessioni fra client.
...
Le classi del componente \textsf{Gestione remota} hanno la cognizione dell'iter di chiamate a metodo per portare a termine correttamente le operazioni richieste dall'esterno. Un esempio di richiesta proveniente dal server che richiede di essere gestita in questa sede è la reazione al cambiamento di stato di un utente presente nella rubrica del client, che si traduce in una notifica in entrata dal server e ha un effetto sull'interfaccia grafica.
