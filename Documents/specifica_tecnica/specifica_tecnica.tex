...
\subsubsection*{Vantaggi}
\begin{itemize}
  \item si tratta di un DBMS \inglese{embedded} integrato strettamente nell'applicazione stessa che non richiede l'esecuzine di un servizio esterno sull'\inglese{host} in cui è eseguito.
\end{itemize}
...
\begin{enumerate}
  \item le classi \inglese{transfer object} (TO) e le relative interfacce;
  \item le classi \inglese{data access object} (DAO) concrete per il DBMS prescelto (JavaDB) e le relative interfacce;
...
%TODO
% Sbaglio o forse il package dao dovrebbe solo mantenere gli oggetti DAO e le factory, non avrebbe più senso lasciare gli oggetti TO nei loro posti "corretti" e cioè
% - MessageTO e IMessage nel package message
% - GroupTO, UserDataTO, IGroup e IUserData nel package abook
% - CallTO e ICall da qualche altra parte?
% E, inoltre, dobbiamo proprio posporre quel suffisso TO?
...
La struttura delle classi che lo compongono mostra una corrispondenza biunivoca con gli elementi grafici presentati nella sezione~\ref{sec:gui} con un \inglese{presenter} per ognuno dei pannelli, cui si aggiunge un oggetto istanza di \texttt{guicontrol.PresenterMediator}. Quest'ultimo incapsula la collaborazione fra i vari \inglese{presenter} e permette di gestire gli eventi generati da un elemento della GUI inviando a uno (o più) \inglese{presenter} i messaggi corrispondenti alle azioni da compiere.
	\item{\scshape\bfseries Diagramma delle classi:}\\
%TODO: questo diagramma deve essere fatto
	\item{\scshape\bfseries Classi utilizzate:}\\
	\begin{itemize}[noitemsep,nolistsep]
...
%TODO: amministrazione della rubrica
% IN ADDRESBOOKPANELPRESENTER
% - aggiungi gruppo
% - rimuovi gruppo
% IN CONTACTPANELPRESENTER
% - assegna un utente a un gruppo
% - rimuovi un utente dal gruppo cui appartiene
% - aggiungi un utente alla rubrica (se non presente in rubrica)
	\end{itemize}
\end{description}
