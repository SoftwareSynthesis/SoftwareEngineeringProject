...
\begin{description}
	...
	\item{\scshape\bfseries UserData - AddressBooks (molti a uno)}: tale associazione rappresenta il legame che intercorre tra una rubrica e i gli utenti presenti in essa. Tale associazione è di tipo molti a uno, con totalità parziale da entrambi i lati.
	
	\item{\scshape\bfseries AddressBooks - UserData (uno a uno)}: Tale associazione rappresenta il proprietario di una rubrica. Tale associazione è del tipo uno a uno con totalità parziale da UserData verso AddressBook mentre ha totalità totale da AddressBook verso UserData.
...
\begin{description}
	...
	\item{\scshape\bfseries UserData - AddressBooks}: tali associazioni si sono risolte inserendo nell'entità AddressBook due chiavi esterne verso UserData.
	
	\item{\scshape\bfseries AddressBooks - Groups}: tale associazione si è risolta inserendo una chiave esterna verso Groups all'interno dell'entità AddressBooks.
...
La chiave sintetiche adottate sono del tipo \texttt{BIGINT} senza segno, il team è consapevole che questo impone un limite di $1,8 \times10^{19}$ al numero di voci che possono essere memorizzate contemporaneamente in ogni entità. Una soluzione da adottarsi in futuro per evitare la perdita di dati potrebbe consistere nello spostare da database a disco parte di queste informazioni.
...
Nucleo logico di tale componente è la classe \texttt{HibernateUtil}, il cui compito è creare una \inglese{factory} per le sessioni verso il database. Si voglia osservare che tali sessioni si basano sul file di configurazione di Hibernate, tra le cui informazioni compare anche la tipologia di di DBMS utilizzata.

Le altre classi che fanno parte  di questo componente si dividono in due categorie:
\begin{itemize}
	\item le classi \inglese{data access object} (DAO). Ossia quelle classi aventi il compito di inoltrare richieste per la modifica e la lettura dei dati (Create Read Update Delete) nelle corrispettive entità del database. Tali classi sono:
	\begin{itemize}
		\item[-] \texttt{dao.CallDAO}
		\item[-] \texttt{dao.GroupDAO}
		\item[-] \texttt{dao.MessageDAO}
		\item[-] \texttt{dao.UserDataDAO}
		\item[-] \texttt{dao.AddressBookDAO}
	\end{itemize}
	
	\item le classi che mappano le entità rappresentate nel database sono dotate di variabili di istanza che corrispondono ai campi dei \inglese{record}. Le operazioni disponibili su questo genere di oggetti comprendono i metodi get/set associati ai campi delle tabelle del database e sono rese disponibili dalle interfacce (una per ogni entità) implementate dalle classi (per chiarezza si informa che tali classi corrispondono a quelle che nella logica di programmazione in Hibernate prendono il nome di \inglese{Transfer Object}, TO\@).
	\begin{itemize}
...
Si voglia osservare che le classi rappresentati i TO\@ vengono ad essere collocate in package diversi da \texttt{.server.dao}. Questo perchè il team ha ritenuto sensato inserirle in contesti maggiormente specializzati sul gestire quella specifica sezione del database. Ecco quindi (per esempio) che la classe rappresentante i messaggi in segreteria verrà ad essere collocata in \texttt{server.message.Message}.
...
\subsubsection{CS02 -- Gestione connessione}\label{sec:gestioneconnessione}
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
...
Tale classe sarà adattata alle firme dei metodi dell'interfaccia \texttt{connection.IChannel} ricorrendo al \inglese{design pattern} Adapter, con l'introduzione della classe \texttt{connection.ChannelAdapter}. 
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
Ad ogni utente del sistema, che corrisponde a un'istanza di \texttt{abook.UserData} è associata una rubrica personale. Tale rubrica è rappresentata da una lista d'istanze del tipo \texttt{abook.IAddressBook}, per la quale il team fornisce un implementazione nominata \texttt{abook.AddressBook}. Per chiarezza possiamo dire che un istanza della classe TO \texttt{abook.AddressBook}, rappresenta una riga della rubrica di un utente.

Come si evince osservando il diagramma, la struttura della componente rispecchia la logica di funzionamento del database. Ecco quindi che ogni istanza di un oggetto di tipo \texttt{abook.IAddressBook} ``apparterrà'' ad un solo \texttt{abook.IUserData}, ed in essa è registrata un istanza di \texttt{abook.UserData} che rappresenta il contatto facente parte della rubrica.
...
Inoltre, i cambiamenti di stato vengono notificati a tutti gli utenti presenti in rubrica che si trovano nello stato \inglese{online}.
	\item{\scshape\bfseries Diagramma delle classi:}
\begin{figure}[H]
  \centering
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
I messaggi in segreteria, che possono essere di natura audio o audio/video, corrispondono alle istanze dalla classe \texttt{message.Message} e sono caratterizzati da un utente mittente, da un destinatario e dalla data di registrazione. La collezione di messaggi aventi un determinato utente come destinatario può essere ottenuta tramite un'operazione dichiarata nell'interfaccia \texttt{abook.IUserData}.

La gestione della segreteria lato client è resa più agevole dall'introduzione della classe \texttt{clientpresenter.kernel.JSMessageBox} che rappresenta una collezione di messaggi, mentre \texttt{clientpresenter.kernel.JSMessage} rende disponibili le medesime operazioni di \texttt{message.IMessage} ma costituisce una rappresentazione ad uso dei client.
...
Per la gestione dello storico delle chiamate lato client, invece, sono state previste le classi \texttt{clientpresenter.kernel.JSCall}, che aderisce al medesimo contratto della sua controparte server.
...
\subsubsection{CS07 -- Façade del server}\label{sec:serverfacade}
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
Tale componente ha il ruolo di offrire ai client una serie di punti di accesso alle funzionalità del sistema server. Dal momento che tale interazione avviene in un contesto distribuito, i punti di accesso ai componenti del server sono stati implementati come \inglese{servlet} Java destinate ad essere eseguite nell'ambiente del \inglese{servlet container} fornito da Apache TomCat.

Per questo motivo, tali Façade saranno realizzate come estensioni della classe di libreria \texttt{javax.servlet.http.HttpServlet}, al fine di creare un sistema di risposta lato server che prenderà in carico le richieste inoltrate dai client.
...
In particolare \texttt{servlet.ConnectionManager} è coinvolto in fase di \inglese{login} e collabora con \textsf{Gestione rubrica} e \textsf{Gestione Database} per controllare la validità dell'autenticazione, quindi interagisce con \textsf{Gestione connessione} per creare un canale di comunicazione fra il server e il client, secondo le modalità illustrate nella sezione \ref{sec:gestioneconnessione}.
...
%Classe rappresentante il \inglese{transfer object} verso il database dell'entità Message. Ogni sua istanza rappresenta una messaggio presente nella segreteria di un utente\@.

\item \texttt{CallDAO}
\begin{description}
  \item{\scshape\bfseries Descrizione:}\\

Tale classe interagisce con il framework Hibernate e contiene la logica per la manipolazione dei dati (in lettura e scrittura) nella tabella Call del database.
...
\item \texttt{AddresBookDAO}
\begin{description}
  \item{\scshape\bfseries Descrizione:}\\

Tale classe interagisce con il framework Hibernate e contiene la logica per la manipolazione dei dati (in lettura e scrittura) nella tabella AddressBook del database.
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
Interfaccia per la gestione delle richieste per ottenere le informazioni necessarie a un client per stabilire una comunicazione con altri client. In particolare le operazioni in essa dichiarate dovranno ritornare un'istanza di un oggetto che implementa l'interfaccia \texttt{IChannel}.
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
Implementazione che si fornisce dell'interfaccia \texttt{ICommunicationHandler}. Tale classe avrà il compito di instaurare una connessione \inglese{full-duplex} con i client che desiderano utilizzare l'applicativo in seguito all'autenticazione, fornendo in risposta l'istanza di un opportuno sottotipo di \texttt{IChannel}.
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
Interfaccia che definisce un canale di comunicazione client-server. Tale canale sarà usato esclusivamente al fine di processare le richieste provenienti dai client (nel caso di una comunicazione in ingresso) o di indirizzare a questi ultimi (nel caso di una comunicazione in uscita) al fine di stabilire una connessione WebRTC client-client.
...
\item \texttt{ChannelAdapter}
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
Classe che risolve l'utilizzo del \inglese{pattern} Adapter tra l'interfaccia \texttt{connection.IChannel} (\inglese{target} dell'adattamento) e la classe \texttt{org.apache.catalina.websocket.MessageInbound} (che corrisponde invece all'\inglese{adaptee}).
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
Interfaccia che raccoglie le operazioni per la gestione di un utente appartenente alla rubrica di un altro utente.
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
Classe che implementa l'interfaccia \texttt{IAddressBook} e rappresenta quindi una voce della rubrica associata a un determinato utente. Permette di accedere al contatto di cui si compone e al gruppo e se tale utente risulta essere in \inglese{blacklist}
...
\subsubsection{Package \texttt{org.softwaresynthesis.mytalk.server.servlet}}
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
Sottoclasse di \texttt{javax.servlet.http.HttpServlet} che ha il compito di fornire ai client un'interfaccia per la consultazione e la gestione della rubrica. Rispondendo alle richieste dei client, infatti, consente lo scaricamento della rubrica restituendo i contatti in una forma serializzata adatta allo scambio di dati.
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
Sottoclasse di \texttt{javax.servlet.http.HttpServlet} che ha la funzione di punto di accesso al componente \textsf{Gestione connessione} grazie al quale, ad autenticazione avvenuta, viene instaurato un canale di comunicazione bidirezionale fra il server e il client, destinato ad essere usato per effettuare e per ricevere le comunicazioni fra client.
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
Sottoclasse di \texttt{javax.servlet.http.HttpServlet} avente la funzione di recepire le richieste corrispondenti ai cambiamenti di stato dei client del sistema, fornendo pertanto un punto d'accesso al componente \textsf{Gestione stato}.

I cambiamenti di stato hanno origine nel client tanto per esplicito intervento dell'utente (ad es. una disconnessione o un passaggio da disponibile a ``occupato'') oppure implicitamente ad esempio alla ricezione di una comunicazione audio o audio/video.
...
Oltre allo scaricamento dei messaggi in forma serializzata adatta allo scambio dei dati, le richieste provenienti dai client prevedono la cancellazione di un messaggio dalla segreteria (RUFF15.4.0) oppure l'impostazione dello stato del messaggio da ``non ascoltato'' ad ``ascoltato'' (RUFF15.5.0).
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
Interfaccia che raccoglie la dichiarazione di tutte le operazioni astratte con cui si suppone che il server realizzi lo scambio di messaggi con il \inglese{presenter}. La sequenza di operazioni da eseguire che interessano i componenti del \inglese{presenter} (ad esempio \textsf{Gestione GUI}) saranno incapsulate nell'implementazione delle operazioni contenute in questa interfaccia.
...
A livello di implementazione del \inglese{pattern}, come esemplificato dal diagramma in figura \ref{fig:adapter}, si è scelto l'utilizzo di un Adapter basato su oggetti, mediante la tecnica di composizione, e non su classi, cioè mediante ereditarietà.

Le ragioni di una simile scelta sono da ravvisarsi nel fatto che il linguaggio Java non consente l'ereditarietà privata e questo avrebbe creato una relazione di sottotipo non voluta fra l'Adapter (\texttt{connection.ChannelAdapter}) e l'\inglese{adaptee} (\texttt{MessageInbound}).

\begin{figure}[H]
  \centering
  \includegraphics[width=.8\textwidth]{pattern_adapter}
  \caption{Applicazione del \inglese{pattern} Adapter}\label{fig:adapter}
\end{figure}
...
Inoltre, poiché si è scelta la variante del \inglese{pattern} basata su oggetti, è possibile utilizzare qualsiasi sottotipo dell'\inglese{adaptee} senza dover modificare nulla dell'Adapter stesso.
\end{description}
...
\subsubsection*{Classe \texttt{org.softwaresynthesis.mytalk.server.servlet.AccountManager}}
...
A tal fine, come illustrato dal diagramma in figura \ref{fig:facadeaddressbook2}, è prevista una collaborazione fra la \inglese{servlet} \texttt{AddressBookManager} e l'interfaccia \texttt{abook.IUserData} e \texttt{abook.IGroup} nonché con \texttt{dao.IUserDataDAO} e \texttt{dao.IGroupDAO} per memorizzare nel database che risiede sul server le nuove informazioni risultanti delle interazioni con l'utente.
...
\subsubsection*{Classe \texttt{org.softwaresynthesis.mytalk.server.servlet.ConnectionManager}}
...
Inoltre, poiché questa \inglese{servlet} è interrogata in fase di \inglese{login} al sistema, ha anche la responsabilità di testare i dati di autenticazione al fine di stabilire se le credenziali sono valide per effettuare l'accesso, prima di creare il canale di comunicazione bidirezionale corrispondente a \texttt{connection.IChannel}.

Come illustrato dal diagramma riportato in figura \ref{fig:facadeconnection}, al fine di realizzare tali funzionalità sono previste le collaborazioni con le interfacce \texttt{abook.IUserData}, \texttt{dao.IUserDataDAO} e con \texttt{connection.ICommunicationHandler}.
...
La \inglese{sevlet} \texttt{StateManager} rappresenta un punto di accesso centralizzato al componente del server \textsf{Gestione stato} permettendo ai client di cambiare il proprio stato.
...
La \inglese{servlet} \texttt{CallManager} ha la responsabilità di esporre ai client alcune delle funzionalità del componente \textsf{Gestione chiamate} per l'interrogazione del database in merito allo storico delle chiamate e scaricamento dei dati corrispondenti.

A tal fine è prevista la collaborazione con le interfacce \texttt{IUserData}, \texttt{ICall} e \texttt{ICallList} e, per l'interrogazione del database anche con le relative interfacce DAO \texttt{IUserDataDAO} e \texttt{ICallDAO}.
...
Ciò garantisce maggiore flessibilità in quanto permetterebbe in futuro di gestire più categorie di \inglese{handler} che restituiscono diversi tipi connessione.
\end{description}
...
Il componente del \inglese{presenter} che gestisce la logica di comunicazione e necessita di interfacciarsi alla sotto-architettura server nel momento in cui è necessario instaurare una connessione in uscita.
...
Il \inglese{pattern} Observer è utile in quanto permette agli utenti di osservare lo stato degli altri, ricevendo in modo automatico e trasparente una notifica nel caso in cui uno di questi ultimi subisse variazioni, essendo ogni utente sia osservato che osservatore.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=.6\textwidth]{pattern_singleton_connessione}
  \caption{Applicazione del \inglese{pattern} Singleton a \textsf{Gestione connessione}}\label{fig:singleton1}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=.9\textwidth]{pattern_state}
  \caption{Applicazione del \inglese{pattern} State}\label{fig:state}
\end{figure}
