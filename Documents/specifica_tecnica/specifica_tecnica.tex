...
1.8 & Aggiornamento vantaggi del framework \inglese{Hibernate} & Andrea Rizzi & Progettista & 2013-02-18\\
1.7 & Aggiornamento vantaggi del framework \inglese{Hibernate} & Andrea Rizzi & Progettista & 2013-02-18\\
1.6 & Aggiornamento vantaggi del framework \inglese{Hibernate} & Andrea Rizzi & Progettista & 2013-02-15\\
1.5 & Aggiornamento vantaggi del framework \inglese{Hibernate} & Stefano Farronato & Progettista & 2013-02-12\\
...
È il componente che si occupa di rappresentare la struttura del database relazionale su cui poggia l'applicativo, tramite esso il sistema potrà quindi effettuare operazione di lettura e scrittura di entità all'interno del database.
...
Le classi che fanno parte  di questo componente sono quelle che costituiscono i \inglese{data access object} (DAO). Tali classi hanno il compito di inoltrare richieste per la modifica e la lettura dei dati (\inglese{Create}, \inglese{Read}, \inglese{Update} e \inglese{Delete}) nelle corrispondenti entità del database.
...
Precisiamo che le classi rappresentanti i \inglese{transfer object} si trovano a essere collocate in package e componenti diversi da \texttt{server.dao}, dal momento che si è ritenuto sensato inserirle in contesti maggiormente specializzati, ad esempio la classe corrispondente ai messaggi in segreteria verrà ad essere collocata in \texttt{server.message}, quella corrispondente alle chiamate in \texttt{server.call}.

In ultima tale componente contiene anche un'interfaccia che dovrebbe essere implementata da ogni oggetto rappresentante un \inglese{Transfer Object}, tale interfaccia è \texttt{IMyTalkObject}, il cui compito è quello di descrivere il comportamento di un generico oggetto serializzabile in formato \texttt{JSON}.
...
Infine sono state predisposte 11 \inglese{servlet} per interagire con le classi qui descritte:
...
Le loro funzionalità riguardano principalmente le possibilità di modificare, creare e cancellare istanze di oggetti del tipo \texttt{transfer object} e quindi riconducibili ad oggetti fisicamente presenti nel database. Le \inglese{servlet} saranno descritte con maggiori dettaglio nella sezione \vref{sec:cs07} (\textsf{CS07 -- Gestione Façade}).
...
Per consentire ai client di accedere alle funzionalità gestite da questo componente sono state predisposte delle \inglese{servlet} che, per loro natura, estendono \texttt{javax.servlet.http.HttpServlet} e che saranno descritte con maggiori dettagli nella sezione \vref{sec:cs07}. Al fine di comunicare le funzionalità proposte viene di seguito riportata la lista delle \inglese{servlet} che comunicano con tale componente:
...
Infine, allo scopo di consentire ai client di accedere alle funzionalità di questo componente, è stata prevista una \inglese{servlet} \texttt{call.servlet.DownloadCallHistoryServlet} che estende \texttt{javax.servlet.http.HttpServlet} il cui ruolo sarà descritto con maggiori dettagli nella sezione \vref{sec:cs07}.
...
In particolare \texttt{connection.ChannelServlet} è coinvolto dopo la fase di \inglese{login}, una volta che è stata accertata la validità dell'autenticazione, interagisce con \textsf{Gestione connessione} allo scopo di creare un canale di comunicazione fra il server e il client, secondo le modalità illustrate nella sezione \ref{sec:cs02}.\footnote{%
...
Il compito di interfacciarsi con \textsf{Gestione rubrica} per recuperare la rubrica associata ad un utente e per riflettere sul server le operazioni di amministrazione della rubrica spetta invece al pacchetto di \inglese{servlet} \texttt{abook.servlet}. Tale \inglese{servlet} risponde anche alle richieste dei client per le azioni di aggiornamento e modifica dell'account personale dell'utente associato al client. Nello specifico si ha:
...
\begin{itemize}
	  ...
	  \item[-] \texttt{abook.servlet.AddressBookDoSearchServlet}: utilizzata per ricercare gli utente contenenti nei parametri nome, cognome o mail, una parola chiave usata per fare la ricerca e passata alla \inglese{servlet};
...
Ai fini del \inglese{login} è utilizzata invece la \inglese{servlet} \texttt{authentication.servlet.LoginServlet}, che ha il compito di interfacciarsi con il sistema di autenticazione e, in particolare, con l'istanza del sottotipo di \texttt{javax.security.auth.spi.LoginModule} in uso per verificare le credenziali. Inoltre come già accennato sono presenti le \inglese{servlet} per il \inglese{logout} \texttt{authentication.servlet.LogoutServlet} e per la registrazione \texttt{authentication.servlet.RegisterServlet}.
...
\begin{description}
  \item{\scshape\bfseries Descrizione:}\\
Tale classe interagisce con il \inglese{framework} Hibernate e contiene la logica per la manipolazione dei dati (in lettura e scrittura) nella tabella \textit{Call} del database.
...
Tale classe interagisce con il \inglese{framework} Hibernate e contiene la logica per la manipolazione dei dati (in lettura e scrittura) nella tabella \textit{Message} del database.
...
Tale classe interagisce con il \inglese{framework} Hibernate e contiene la logica per la manipolazione dei dati (in lettura e scrittura) nella tabella \textit{Group} del database.
...
Tale classe interagisce con il \inglese{framework} Hibernate e contiene la logica per la manipolazione dei dati (in lettura e scrittura) nella tabella \textit{UserData} del database.
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
\textit{Servlet} il cui compito consiste nell'aggiungere alla rubrica personale dell'utente un nuovo contatto tra quelli registrati al sistema \caName.
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
\textit{Servlet} il cui compito consiste nel rimuovere dalla rubrica personale dell'utente un contatto tra quelli presenti nella rubrica stessa.
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
\textit{Servlet} il cui compito consiste nel bloccare un contatto presente nella rubrica dell'utente, in questo modo lo stesso non può essere più contattato dal contatto.
	\item{\scshape\bfseries Componenti che ne fanno uso:}
	\begin{itemize}[noitemsep,nolistsep]
	  \item[-] \textsf{CS03 -- Gestione rubrica }
	  \item[-] \textsf{CS07 -- Façade del server}
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
\textit{Servlet} il cui compito consiste nello sbloccare un contatto presente nella rubrica dell'utente, in questo modo lo stesso può essere contattato dal contatto che era stato in precedenza bloccato.
	\item{\scshape\bfseries Componenti che ne fanno uso:}
	\begin{itemize}[noitemsep,nolistsep]
	  \item[-] \textsf{CS03 -- Gestione rubrica }
	  \item[-] \textsf{CS07 -- Façade del server}
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
\textit{Servlet} il cui compito consiste inserire un nuovo gruppo in cui inserire gli utenti presenti nella rubrica personale dell'utente.
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
\textit{Servlet} il cui compito consiste eliminare un gruppo creato in precedenza e quindi presente nella rubrica personale dell'utente.
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
\textit{Servlet} il cui compito consiste nell'inserire un contatto presente nella rubrica dell'utente ad un gruppo esistente nella rubrica personale.
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
\textit{Servlet} il cui compito consiste rimuovere un contatto presente nella rubrica dell'utente da un gruppo esistente nella rubrica personale.
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
\textit{Servlet} il cui compito consiste nel fornire la rubrica di un utente iscritto al sistema \caName.
...
\item \texttt{AccountSettingsServlet}
\begin{description}
  \item{\scshape\bfseries Descrizione:}
\textit{Servlet} che ha il compito di permettere la modifica dei dati personali di un utente iscritto al sistema \caName.
...
\subsubsection{Package \texttt{org.softwaresynthesis.mytalk.server.message.servlet}}

\begin{itemize}[noitemsep,nolistsep]

\item \texttt{InsertMessageServlet}
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
Sottoclasse di \texttt{javax.servlet.http.HttpServlet} che rappresenta un punto di accesso al componente \textsf{Gestione segreteria} e nello specifico consente di inserire un nuovo messaggio nella segreteria di un utente del sistema.
...
\item \texttt{UpdateStatusMessageServlet}
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
Sottoclasse di \texttt{javax.servlet.http.HttpServlet} che rappresenta un punto di accesso al componente \textsf{Gestione segreteria} e nello specifico consente di modificare lo stato di un messaggio presente nella segreteria utente.
...
\item \texttt{DeleteMessageServlet}
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
Sottoclasse di \texttt{javax.servlet.http.HttpServlet} che rappresenta un punto di accesso al componente \textsf{Gestione segreteria} e nello specifico consente l'eliminazione di un messaggio presente nella segreteria del client che richiede l'operazione di cancellazione.
...
\item \texttt{DownloadMessageListServlet}
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
Sottoclasse di \texttt{javax.servlet.http.HttpServlet} che rappresenta un punto di accesso al componente \textsf{Gestione segreteria} e nello specifico permette di scaricare la lista dei messaggi.
...
\subsubsection{Package \texttt{org.softwaresynthesis.mytalk.server.call.servlet}}

\begin{itemize}[noitemsep,nolistsep]

\item \texttt{DownloadCallHistoryServlet}
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
...
\textit{Servlet} che ha il compito di autorizzare l'accesso al sistema \caName di un determinato utente precedentemente iscritto. 
  \item{\scshape\bfseries Componenti che ne fanno uso}
  \begin{itemize}[noitemsep,nolistsep]
    \item[-] \textsf{CS04 -- Gestione autenticazione} %TODO INSERIRE/VERIFICARE COMPONENTI CHE NE FANNO USO
  \end{itemize}
\end{description}
...
\begin{description}
  \item{\scshape\bfseries Descrizione}\\
\textit{Servlet} usata per garantire la registrazione di un utente al sistema \caName.
  \item{\scshape\bfseries Componenti che ne fanno uso}
  \begin{itemize}[noitemsep,nolistsep]
    \item[-] \textsf{CS04 -- Gestione autenticazione}
  \end{itemize}
...
Questo componente ha inoltre il compito di recuperare i dati presenti sul server relativi alla rubrica di un utente, la segreteria telefonica e lo storico delle chiamate che lo interessano. I diversi \inglese{presenter} del componente \textsf{Gestione GUI} hanno dunque anche il compito di interrogare il server al fin di procurarsi le informazioni con cui popolare l'interfaccia grafica utente.
...
Per quanto riguarda l'autenticazione, è prevista la presenza di un \inglese{presenter} per l'interfaccia di \inglese{login}, corrispondente all'oggetto \texttt{guicontrol.LoginPanelPresenter}. Anche per la registrazione vale la stessa considerazione, e tale funzionalità è destinata alla classe \texttt{guicontrol.RegisterPanelPresenter}
...
\begin{description}
  \item{\scshape\bfseries Descrizione:}\\
Classe avente il compito di rappresentare i gruppi della rubrica lato \inglese{presenter}, generato serializzando esclusivamente la parte che si vuole trasmettere ai client delle istanze del \inglese{transfer object} descritto sopra (\texttt{org.softwaresynthesis.mytalk.server.abook.Group}).

In particolare di un gruppo è possibile ottenere il nome e l'elenco dei contatti appartenenti al gruppo.

Gli oggetti \texttt{JSGroup} vengono creati a seguito di una richiesta proveniente dal \inglese{presenter} \texttt{guicontrol.GroupPanelPresenter} e sono successivamente impiegati  al fine di popolare l'interfaccia grafica con la lista dei gruppi utente.
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
Si tratta del \inglese{presenter} richiamato dall'elemento grafico corrispondente al pannello \texttt{org.softwaresynthesis.mytalk.clientview.AccountSettingPanel} per la gestione dei dati relativi all'utente associato al client.

Per questo motivo, l'oggetto \texttt{AccountSettingsPanelPresenter} ha il compito di mettere a disposizione per la vista le funzioni di \inglese{callback} che comportano la modifica di questi stessi dati. 
...
Poiché da \texttt{org.softwaresynthesis.mytalk.clientview.ContactPanel} è possibile anche svolgere alcune attività di amministrazione della rubrica utente, in questo \inglese{presenter} sono presenti anche le funzioni di \inglese{callback} necessarie per l'aggiunta e la rimozione di un contatto particolare ad un gruppo, bloccare o sbloccare tale contatto, nonché per l'aggiunta dello stesso alla rubrica se il suo profilo è stato aperto a seguito di una ricerca fra gli utenti registrati nel sistema.
...
Per questo motivo, l'oggetto \texttt{SearchResultPanelPresenter} ha il compito di mettere a disposizione per la vista le funzioni di \inglese{callback} che comportano la modifica della stringa di ricerca di uno specifico contatto nella lista.
...
In questo pannello sono anche presenti le funzionalità di ricerca fra gli utenti presenti nel sistema, con la visualizzazione dei risultati della ricerca sostituita alla lista dei contatti presenti in rubrica.
...
Le classi DAO consentono di isolare l'accesso alle tabelle del database dalla parte di \inglese{business logic} corrispondente alle entità TO (\textit{transfer object}), incapsulando in metodi di alto livello le operazioni sui record del database sottostante.
...
Questa componente della sotto-architettura server fornisce una serie di punti di accesso centralizzati (\inglese{servlet}) per i componenti \textsf{Gestione rubrica}, \textsf{Gestione connessione}, \textsf{Gestione segreteria}, \textsf{Gestione chiamate} e \textsf{Gestione autentcazione} secondo la tabella riportata nella sezione \vref{sec:cs07}. In pratica è possibile pensare ad ogni \inglese{servlet} come ad un façade atto a nascondere l'iter procedurale per la realizzazione di una determinata operazione.
\end{description}
...
\begin{figure}[H]
  \centering
   \includegraphics[width=.7\textwidth]{pattern_facade_rubrica}
  \caption{Applicazione del \inglese{pattern} Façade a \textsf{Gestione rubrica}}\label{fig:facadeaddressbook1}
\end{figure}

Essendo già il diagramma sufficientemente complesso si ritiene che l'aggiunta di tutte le frecce che rappresentano le dipendenza lo rendano completamente illeggibile, per tale ragione le dipendenze si riportano in forma di elenco, in particolare:
\begin{itemize}[noitemsep,nolistsep,leftmargin=.5em]
  \item[-] la \inglese{servlet} \texttt{AddressBookDoAddContactsServlet} ha una dipendenza da \texttt{AddressBookEntry}, \texttt{IGroup}, \texttt{IUserData}, \texttt{GroupDAO} e \texttt{UserDataDAO};
  ...
  \item[-] la \inglese{servlet}  \texttt{AddressBookDoDeleteGroupServlet} dipende invece da \texttt{IAddressBookEntry}, \texttt{IGroup}, \texttt{AddressBookEntryDAO} e \texttt{GroupDAO};
  ...
  \item[-] la \inglese{servlet} \texttt{AddressBookDoRemoveContactServlet} dipende da \texttt{IAddressBookEntry}, \texttt{IUserData}, \texttt{AddressBookEntryDAO} e \texttt{UserDataDAO};
...
\subsubsection*{Classi del package \texttt{org.softwaresynthesis.mytalk.server.message.servlet}}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=.7\textwidth]{pattern_facade_segreteria}
  \caption{Applicazione del \inglese{pattern} Façade a \textsf{Gestione segreteria}}\label{fig:facadeaddressbook2}
\end{figure}
...
\begin{figure}[H]
  \centering
     \includegraphics[width=.7\textwidth]{pattern_facade_connessione}
...
Tale classe nasconde la complessità di gestione di una richiesta di \inglese{download} della lista delle chiamate. Nello specifico svincola l'utilizzatore dal dover conoscere l'iter di utilizzo delle classi di \texttt{server.abook}, \texttt{server.dao} e \texttt{server.call}.

\begin{figure}[H]
  \centering
   \includegraphics[width=.7\textwidth]{pattern_facade_chiamate}
  \caption{Applicazione del \inglese{pattern} Façade a \textsf{Gestione chiamate}}\label{fig:facadestate}
\end{figure}

\subsubsection*{Classi del package \texttt{org.softwaresynthesis.mytalk.server.authentication.servlet}}

Ogni classe di tale package implementa Façade poiché nasconde l'iter per l'autenticazione (\inglese{login}, \inglese{logout}  e registrazione) di un utente svincolando l'utilizzatore dal doverlo conoscere. Nello specifico tali classi richiamano in più punti e in maniera sequenziale, classi presenti nel package \texttt{server.abook}, \texttt{server.dao} e \texttt{server.autenthication}.

\begin{figure}[H]
  \centering
  \includegraphics[width=.7\textwidth]{pattern_facade_autenticazione}
  \caption{Applicazione del \inglese{pattern} Façade a \textsf{Gestione autenticazione}}\label{fig:facademessage}
\end{figure}
