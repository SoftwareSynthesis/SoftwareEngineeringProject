...
\end{center}
	...
	\item{\scshape\bfseries AddressBookEntries}: è l'entità le cui istanze rappresentano gli utenti  appartenenti alla rubrica di un utente. AddressBookEntries è caratterizzata dai seguenti attributi:
...
\begin{description}
	...
	\item{\scshape\bfseries UserData - AddressBookEntries (molti a uno)}: tale associazione rappresenta il legame che intercorre tra una rubrica e i gli utenti presenti in essa. Tale associazione è di tipo molti a uno, con totalità parziale da entrambi i lati.
	
	\item{\scshape\bfseries AddressBookEntries - UserData (uno a uno)}: Tale associazione rappresenta il proprietario di una rubrica. Tale associazione è del tipo uno a uno con totalità parziale da UserData verso AddressBookEntries mentre ha totalità totale da AddressBookEntries verso UserData.
	
	\item{\scshape\bfseries AddressBookEntries - Groups (molti a uno)}: tale associazione rappresenta la possibilità di suddividere la propria rubrica in gruppi. Tale associazione è del tipo molti a uno, con totalità totale da Groups ad AddressBookEntries mentre ha una totalità parziale da AddressBookEntries a Groups.
...
%TODO da rivedere!
\subsubsection{CS04 -- Gestione autenticazione}
\begin{description}
  \item{\scshape\bfseries Descrizione:}\\
L'autenticazione passa attraverso due fasi: la richiesta di \inglese{login} e l'approvazione (\inglese{commit}). La richiesta di \inglese{login} verifica le credenziali di accesso, mentre la richiesta di \inglese{commit} permette di avere accesso all'entità utilizzata nel resto del programma.

Responsabile di entrambe le fasi è l'unica istanza di \texttt{authentication.AuthenticationModule}, che si occupa di caricare le credenziali di autenticazione creando un'istanza di \texttt{authentication.CredentialLoader}. In seguito, se il \inglese{login} va a buon fine, crea un'istanza di \texttt{javax.security.auth.Subject} da cui è possibile accedere ai \texttt{javax.security.auth.Principal} (in particolare \texttt{authentication.PrincipalImpl}) che permettono di identificare in maniera univoca gli utenti del sistema.
...
CS03 -- Gestione rubrica & servlet.AddressBookManager servlet.AccountManager servlet.StateManager\\
...
Tale classe interagisce con il framework Hibernate e contiene la logica per la manipolazione dei dati (in lettura e scrittura) nella tabella AddressBookEntries del database.
...
\item \texttt{CommunicationHandler}
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
Interfaccia che definisce un canale di comunicazione client-server. Tale canale sarà usato esclusivamente al fine di processare le richieste provenienti dai client (nel caso di una comunicazione in ingresso) o di indirizzarle a questi ultimi (nel caso di una comunicazione in uscita) al fine di stabilire una connessione WebRTC client-client.
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
Classe che implementa il \inglese{design pattern} Adapter, adattando la classe \texttt{org.apache.catalina.websocket.MessageInbound} (che corrisponde all'\inglese{adaptee}) all'interfaccia \texttt{connection.IChannel} (\inglese{target}).
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
Classe che implementa l'interfaccia \texttt{IAddressBookEntry} e rappresenta quindi una voce della rubrica associata a un determinato utente.
...
\begin{description}
	\item{\scshape\bfseries Descrizione:}\\
Interfaccia per i gruppi interni alla rubrica di un utente, prevede un'operazione astratta \texttt{add(IUserData)} per l'aggiunta di un nuovo contatto al gruppo e un'operazione \texttt{remove(IUserData)} per la sua rimozione di un contatto dal gruppo. Contiene anche i metodi \inglese{get}/\inglese{set} per impostare o recuperare il nome di un gruppo.
...
\begin{description}
  \item{\scshape\bfseries Descrizione:}\\
Classe che attraverso un tipo enumerativo rappresenta lo stato in cui si trova un'istanza di un sottotipo di \texttt{abook.IUserData}. Le istanze della classe \texttt{abook.UserData} sono composte con un'istanza di \texttt{abook.State} che rappresenta lo stato in cui si trova un determinato utente.

In una prima realizzazione del prodotto è stato scelto di implementare i seguetni stati:
\begin{itemize}
  \item \inglese{offline};
  \item \inglese{online}/disponibile;
  \item \inglese{online}/occupato.
\end{itemize}
...
%TODO cosa fa questo affare resta da chiarire
...
%TODO non dovremmo mettere anche il componente CP03 Gestione GUI su tutti questi? manipolano la vista
\item \texttt{AddressBookPanel}
\begin{description}
  \item{\scshape\bfseries Descrizione}\\
...
\subsubsection{Componenti che lo implementano}
\begin{description}
%TODO deve diventare gestione connessione oppure gestione rubrica
