...
La mappatura fra componenti del server e i rispettivi \inglese{controller} è illustrata dalla tabella seguente:
...
\begin{itemize}
	  ...
	  \item[-] \texttt{abook.controller.SearchController}: utilizzata per ricercare gli utente contenenti nei parametri nome, cognome o mail, una parola chiave usata per fare la ricerca e passata alla \inglese{servlet};
...
Il diagramma in figura X rappresenta l'implementazione generica del front controller che, come anticipato, nasconde agli utilizzatori l'iter per la gestione dei specifici servizi (rubrica, segreteria, autenticazione, etc.). Per ogni servizio è descritta con maggiori dettagli e la relativa rappresentazione grafica, l'applicazione del \inglese{design pattern} nel paragrafo \vref{sec:patternfacade}.
...
Poiché inoltre da \texttt{org.softwaresynthesis.mytalk.clienview.ToolsPanel} è possibile impostare il proprio stato, il \inglese{presenter} ad esso associato contiene le funzioni che notificano l'avvenuto cambiamento di stato al componente \textsf{Gestione controller}.
...
\begin{description}
  \item{\scshape\bfseries Descrizione}\\
Implementazione di \texttt{IAddressBookPanel}.
...
\begin{description}
  \item{\scshape\bfseries Descrizione}\\
Implementazione di \texttt{IMainPanel}
...
\begin{description}
  \item{\scshape\bfseries Descrizione}\\
Implementazione di \texttt{IToolsPanel}.
...
\begin{description}
  \item{\scshape\bfseries Descrizione}\\
Pannello che estende \texttt{IMainPanel} utilizzato per rappresentare il profilo di un utente. Solo accedendo quest'ultimo sarà possibile avviare una comunicazione con l'utente. Il contatto viene visualizzato selezionando l'utente dalla rubrica oppure tra i risultati di una ricerca.
...
\begin{description}
  \item{\scshape\bfseries Descrizione}\\
Implementazione di \texttt{IContactPanel}.
...
\begin{description}
  \item{\scshape\bfseries Descrizione}\\
Questa Interfaccia, che estende \texttt{IMainPanel}, permette di accedere all'elenco dei messaggi in segreteria di un determinato utente e ne permette la gestione. La visualizzazione si attiva quando viene premuto il relativo pulsante mostrato dall'istanza di \texttt{ToolsPanel}.
...
\begin{description}
  \item{\scshape\bfseries Descrizione}\\
Implementazione di \texttt{IMessagePanel}.
...
\begin{description}
  \item{\scshape\bfseries Descrizione}\\
Implementazione di \texttt{IGroupPanel}.
...
\begin{description}
  \item{\scshape\bfseries Descrizione}\\
Implementazione di \texttt{ISearchPanel}.
...
\begin{description}
  \item{\scshape\bfseries Descrizione}\\
Implementazione di \texttt{IAccountSettingPanel}.
...
\begin{description}
  \item{\scshape\bfseries Descrizione}\\
Implementazione di \texttt{ICallHistoryPanel}.
...
\begin{description}
  \item{\scshape\bfseries Descrizione}\\
Implementazione di \texttt{CommunicationPanel}.
...
\begin{description}
  \item{\scshape\bfseries Descrizione}\\
Implementazione di \texttt{ILoginPanel}.
...
\begin{description}
  \item{\scshape\bfseries Descrizione}\\
Implementazione di \texttt{IPanelPresenter}.
...
Questa componente della sotto-architettura server fornisce una serie di punti di accesso centralizzati (\inglese{servlet}) per i componenti \textsf{Gestione rubrica}, \textsf{Gestione connessione}, \textsf{Gestione segreteria}, \textsf{Gestione chiamate} e \textsf{Gestione autenticazione} secondo la tabella riportata nella sezione \vref{sec:cs07}. In pratica è possibile pensare ad ogni \inglese{controller} come ad un façade atto a nascondere l'iter procedurale per la realizzazione di una determinata operazione.
\end{description}
...
\begin{figure}[H]
  ...
  \caption{Applicazione del \inglese{pattern} Front Controller a \textsf{Gestione rubrica}}\label{fig:facadeaddressbook1}
\end{figure}


%VA CANCELLATO QUESTO PEZZO: VENGONO IMPLEMENTATE LE CENTINAIA DI FRECCE NEL DIAGRAMMA
...
\begin{itemize}[noitemsep,nolistsep,leftmargin=.5em]
  \item[-] il \inglese{controller} \texttt{AddContactController} ha una dipendenza da \texttt{AddressBookEntry}, \texttt{IGroup}, \texttt{IUserData}, \texttt{dao.DataPersistanceManager} e \texttt{UserData};
...
\subsubsection*{Classi del package \texttt{org.softwaresynthesis.mytalk.server.message.controller}}
...
\begin{figure}[H]
  ...
  \caption{Applicazione del \inglese{pattern} Front Controller a \textsf{Gestione segreteria}}\label{fig:facadeaddressbook2}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Applicazione del \inglese{pattern} Front Controller a \textsf{Gestione connessione}}\label{fig:facadeconnection}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Applicazione del \inglese{pattern} Front Controller a \textsf{Gestione chiamate}}\label{fig:facadestate}
\end{figure}

\subsubsection*{Classi del package \texttt{org.softwaresynthesis.mytalk.server.authentication.servlet}}

Ogni classe di tale package implementa Front Controller poiché nasconde l'iter per l'autenticazione (\inglese{login}, \inglese{logout}  e registrazione) di un utente svincolando l'utilizzatore dal doverlo conoscere. Nello specifico tali classi richiamano in più punti e in maniera sequenziale, classi presenti nel package \texttt{server.abook}, \texttt{server.dao} e \texttt{server.autenthication}.

\begin{figure}[H]
  \centering
  \includegraphics[width=.7\textwidth]{pattern_facade_autenticazione}
  \caption{Applicazione del \inglese{pattern} Front Controller a \textsf{Gestione autenticazione}}\label{fig:facademessage}
\end{figure}
