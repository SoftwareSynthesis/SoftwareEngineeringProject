...
\begin{tabular}{lccc}
\toprule
Nome attributo & Tipo & Opzionale & Chiave\\
\midrule % inserire qui il contenuto della tabella
...
\begin{description}
	\item{\scshape\bfseries UserData - Groups}: tale associazione rappresenta il legame che intercorre tra gli utenti ed i gruppi, per la quale si è deciso di usare un associazione molti a molti con totalità parziale da entrambi i lati. Ciò significa che: un utente può comparire in uno o più gruppi (in ragione del fatto che potrebbe esserci qualcuno che registra l'utente A nella propria rubrica), ed un utente può possedere più gruppi.
...
\begin{description}
	\item{\scshape\bfseries UserData - Groups}: tale associazione si è risolta con la creazione di una terza tabella denominata UserData\_Group, che contiene una chiave esterna verso UserData e una chiave esterna verso Groups.
