...
Infine, dal momento che gli utenti del sistema sono dotati di uno stato, è prevista la presenza di un'enumerazione \texttt{abook.State} all'interno delle istanze di \texttt{abook.UserData}.
...
Per permettere agli utenti dei sistema di osservare lo stato degli altri utenti e di notificare agli altri i cambiamenti del proprio, ad ogni cambio di stato, l'utente non farà altro che notificare l'evento alla servlet \texttt{connection.ConnectionManager}. Quest'ultima si preoccuperà di aggiornare gli utenti aventi in rubrica l'utente il cui stato è cambiato.
