...
al termine della procedura la schermata principale di \textit{TomCat} dovrà essere equivalente alla seguente se tutto è andato a buon fine; Nel caso il vostro tomcat esegua solo il deploy e non l'avvio eseguite tale operazione manualmente cliccando sull'apposito pulsante \textit{start} riportato nella riga raffigurante l'applicazione \textit{MyTalk}.\\
In caso di errori di \textit{deploy} e/o di avvio dell'applicativo essi sono riportati nella prima riga in alto della pagina; si prega di segnalare tale errore al team di sviluppo che provvederà nel più breve tempo possibile alla risoluzione del problema.
...
\item verificare infine che il proprio server cosi configurato sia visibile dalla rete.
%TODO: proseguire specificando meglio
\end{enumerate}
