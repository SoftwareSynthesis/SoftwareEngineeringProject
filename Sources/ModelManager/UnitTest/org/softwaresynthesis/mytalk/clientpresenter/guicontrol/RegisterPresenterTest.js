module(
		"RegisterPanelPresenterTest",
		{
			setup : function() {
				...
				/*
				 * oggetto da testare, lo script PHP che sta al posto della
				 * servlet LoginManager è anch'esso, ebbene sì, uno stub!
				 */
...
test(
		...
		function() {
			...
			i++;
			var form = children[0];
			equal(form.nodeName, "FORM", "l'elemento è un <form>");
			i++;

			equal(form.getAttribute("enctype"), "multipart/form-data",
					"attributo enctype del form impostato correttamente");
			i++;
			...
			equal(labelUsername.nodeName, "LABEL",
					"la label ha il tipo corretto");
			i++;
			equal(labelUsername.getAttribute("for"), inputUsername.id,
					"attributo for della label corretto");
			i++;
			equal(inputUsername.type, "email",
					"l'input username ha il tipo corretto");
			i++;
			ok(inputUsername.required,
					"attributo required corretto per username");
			i++;
			...
			equal(labelPassword.getAttribute("for"), inputPassword.id,
					"la label ha l'attributo for corretto");
			i++;
			equal(inputPassword.type, "password",
					"tipo corretto dell'input per la password");
			i++;
			...
			equal(inputSecretQ.type, "text",
					"tipo corretto dell'input per la domanda segreta");
			i++;
			ok(inputSecretQ,
					"attributo required corretto per la domanda segreta");
			i++;
			...
			equal(inputAnswerSQ.type, "text",
					"tipo corretto dell'input per la risposta alla domanda segreta");
			i++;
			ok(inputAnswerSQ,
					"attributo required corretto per la risposta alla domanda segreta");
			i++;
			...
			i++;
			// trovato errore: nell'attributo 'for' del '<label>' era scritto
			// firtname invece di firstname ;)
			equal(labelName.getAttribute("for"), inputName.id,
					"attributo for della label corretto");
			i++;
			equal(inputName.type, "text",
					"l'input per il nome ha il tipo corretto");
			i++;
			...
			equal(inputSurname.type, "text",
					"l'input per il cognome ha il tipo corretto");
			i++;
			ok(!inputSurname.required,
					"attributo required corretto per il cognome");
			i++;
			...
			equal(inputPicture.type, "file",
					"l'input per l'immagine ha il tipo corretto");
			i++;
			ok(!inputPicture.required,
					"attributo required corretto per l'immagine");
			i++;
			...
			equal(button.nodeName, "BUTTON", "elemento pulsante corretto");
			i++;
			equal(button.type, "submit", "il pulsante è presente");
			i++;
...
test("testHide()", function() {
	...
	equal(element.style.display, "none",
			"il pannello è stato correttamente nascosto");
});

// richiede di essere in un WEB SERVER per essere eseguito (altrimenti non
// interpreta lo script PHP)
test(
		"testRegister()",
		function() {
			var i = 0;
			// questo è uno stub (simula i dati recuperati dal form
			// dell'interfaccia)
			var data = {
				name : "Flavia",
				surname : "Bacco",
				username : "flabacco@gmail.com",
				password : "farfalla",
				picturePath : "flavietta.png",
			...
			equal(
					...
					"stringa di query generata correttamente");
			i++;
			equal(communicationcenter.my.name, "Flavia",
					"nome registrato correttamente");
			i++;
			equal(communicationcenter.my.surname, "Bacco",
					"cognome registrato correttamente");
			i++;
			equal(communicationcenter.my.username, "flabacco@gmail.com",
					"username registrato correttamente");
			i++;
			equal(communicationcenter.my.id, 0,
					"id del nuovo utente impostato correttamente");
			i++;
			equal(communicationcenter.my.picturePath,
					"http://softwaresynthesis.org/pictures/flavietta.png",
					"immagine del nuovo utente salvata correttamente");
			i++;
...
test("testGetPicturePath()", function() {
	...
	equal(picturePath, inputPicture.value,
			"percorso dell'immagine recuperato correttamente");
...
test("testGetUsername()", function() {
	...
	try {
		...
		equal(err, "indirizzo email non specificato",
				"username mancante rilevato correttamente");
		i++;
	}
	...
	try {
		...
		equal(err, "indirizzo email non valido",
				"email malformato rilevato correttamente");
		i++;
	}
...
test("testGetQuestion()", function() {
	...
	equal(question, inputSecretQ.value,
			"domanda segreta recuperata correttamente");
	i++;
	...
	try {
		...
		equal(err, "domanda segreta non specificata",
				"domanda segreta mancante rilevata correttamente");
		i++;
	}
...
test("testGetAnswer()", function() {
	...
	try {
		...
		equal(err, "risposta alla domanda segreta non specificata",
				"risposta mancante rilevata correttamente");
		i++;
	}
